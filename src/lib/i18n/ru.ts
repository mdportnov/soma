export const ru = {
  // Navigation
  nav: {
    dashboard: "Главная",
    timeline: "Временная шкала",
    assistant: "Soma AI",
    biomarkers: "Биомаркеры",
    labResults: "Анализы",
    medications: "Лекарства",
    visits: "Визиты",
    diagnoses: "Диагнозы",
    allergies: "Аллергии",
    vaccines: "Прививки",
    imaging: "Снимки",
    journal: "Дневник",
    notes: "Заметки",
    lifestyle: "Образ жизни",
    notifications: "Уведомления",
    report: "Отчёт для врача",
    settings: "Настройки",
    records: "Записи",
    labsVitals: "Анализы и показатели",
    care: "Лечение",
    showAll: "Показать все разделы",
    showFewer: "Свернуть",
    manageSections: "Настроить разделы",
    aiNeedsSetup: "Настроить",
  },

  // AI-анализ (v0.3)
  aiAnalysis: {
    title: "Soma AI",
    description: "Спросите о своей карте или добавьте данные о здоровье обычным текстом.",
    stubTitle: "AI выключен",
    stubDescription: "Добавьте API-ключ в настройках, чтобы включить AI-анализ.",
    openSettings: "Открыть настройки",
    empty: "Спросите о своей карте или расскажите о лекарстве, диагнозе, симптоме либо измерении.",
    starter1: "Что говорят мои последние анализы?",
    starter2: "Какие показатели вне нормы и почему это важно?",
    starter3: "Есть ли взаимодействия между моими лекарствами?",
    placeholder: "Спросите о своём здоровье…",
    inputHint:
      "Enter — отправить, Shift+Enter — новая строка. Не является медицинской консультацией.",
    attachDocument: "Импортировать медицинский документ",
    importDocument: "Импорт",
    thinking: "Думаю…",
    retry: "Повторить",
    stop: "Остановить",
    clear: "Очистить чат",
    clearConfirmTitle: "Очистить чат?",
    clearConfirmBody:
      "Переписка уйдёт в архив, ассистент начнёт с чистого листа. Записи, уже сохранённые в медицинскую карту, не затрагиваются.",
    jumpToLatest: "К последнему",
    regenerate: "Переспросить",
    copied: "Скопировано",
    viewContext: "Что видит ассистент",
    contextExplainer:
      "Эта safety-сводка отправляется с каждым вопросом. Остальные записи ассистент извлекает только при необходимости. Имя не передаётся.",
    copyAnswer: "Скопировать ответ",
    regenerateAnswer: "Спросить заново",
    regenerateBlocked: "Дождитесь ответа или остановите его",
    showEarlier: "Показать раньше",
    threads: {
      title: "Чаты",
      new: "Новый чат",
      newUntitled: "Новый чат",
      search: "Найти чат",
      noMatches: "Ничего не нашлось по «{{query}}»",
      empty: "Здесь будут ваши чаты",
      today: "Сегодня",
      yesterday: "Вчера",
      lastWeek: "За неделю",
      earlier: "Раньше",
      archived: "Архив",
      current: "Открытый чат",
      menu: "Действия с чатом",
      rename: "Переименовать",
      renamePlaceholder: "Название чата",
      archive: "В архив",
      restore: "Вернуть из архива",
      delete: "Удалить",
      archivedToast: "Чат убран в архив",
      restoredToast: "Чат возвращён",
      deletedToast: "Чат удалён",
      undo: "Вернуть",
      details: "О чате",
      recordGone: "записи больше нет",
      created: "Начат",
      lastActivity: "Последняя активность",
      messages: { one: "{{n}} сообщение", few: "{{n}} сообщения", many: "{{n}} сообщений" },
      model: "Модель",
      modelsUsed: "Какие модели отвечали",
      replies: { one: "{{n}} ответ", few: "{{n}} ответа", many: "{{n}} ответов" },
      recordsCited: "На какие записи опирался ассистент",
      recordsChanged: "Что изменилось в карте из этого чата",
      noRecords:
        "Пока ничего: записи появятся, когда ответ сошлётся на них или вы сохраните изменения.",
      changesSaved: {
        one: "{{n}} изменение сохранено",
        few: "{{n}} изменения сохранено",
        many: "{{n}} изменений сохранено",
      },
      pending: {
        one: "{{n}} черновик ждёт решения",
        few: "{{n}} черновика ждут решения",
        many: "{{n}} черновиков ждут решения",
      },
      toolCalls: {
        one: "{{n}} обращение к записям",
        few: "{{n}} обращения к записям",
        many: "{{n}} обращений к записям",
      },
      records: { one: "{{n}} запись", few: "{{n}} записи", many: "{{n}} записей" },
      hits: { one: "упомянута 1 раз", few: "упомянута {{n}} раза", many: "упомянута {{n}} раз" },
      switchBlocked: "Дождитесь ответа или остановите его, прежде чем переключать чат",
      showPanel: "Показать чаты",
      hidePanel: "Скрыть чаты",
      listError: "Не удалось загрузить чаты.",
      retry: "Попробовать ещё раз",
    },
    changes: {
      saved: "Сохранено в медицинскую карту",
      itemCount: "записей: {{count}}",
      showDetails: "Подробности",
      hideDetails: "Свернуть",
      discarded: "Черновик удалён",
      review: "Ничего не сохранится, пока вы не проверите и не подтвердите",
      saving: "Сохраняю…",
      save: "Сохранить выбранное ({{count}})",
      discard: "Удалить черновик",
      blocked: "Исключите заблокированные пункты или уточните недостающие данные в чате.",
      matches: "Найдены похожие записи",
      openRecord: "Открыть запись",
      risk: {
        standard: "Обычная запись",
        elevated: "Медицинская карта",
        destructive: "Удаление",
      },
      operation: {
        create: "Добавить",
        update: "Изменить",
        end: "Завершить",
        merge: "Дополнить",
        delete: "Удалить",
      },
      field: {
        activityLevel: "Активность",
        administeredBy: "Кто ввёл",
        alcohol: "Алкоголь",
        allergen: "Аллерген",
        armSide: "Рука",
        asNeeded: "По необходимости",
        batchNumber: "Номер партии",
        birthDate: "Дата рождения",
        bloodType: "Группа крови",
        bodyArea: "Область тела",
        category: "Категория",
        citizenship: "Гражданство",
        city: "Город",
        clinic: "Клиника",
        country: "Страна",
        date: "Дата",
        datePrecision: "Точность даты",
        dateRaw: "Дата как в источнике",
        diastolic: "Диастолическое",
        doctorName: "Врач",
        doseAmount: "Доза",
        doseNumber: "Номер дозы",
        doseUnit: "Единица дозы",
        effectiveDate: "Дата вступления",
        emergencyNotes: "Заметки для экстренных случаев",
        endDate: "Дата окончания",
        energyLevel: "Уровень энергии",
        ethnicity: "Этническая принадлежность",
        expiresAt: "Истекает",
        fields: "Поля",
        findings: "Заключение",
        frequency: "Частота",
        heartRateBpm: "Пульс",
        heightCm: "Рост",
        icdCode: "Код МКБ",
        intervalMonths: "Интервал (мес.)",
        items: "Позиции",
        label: "Название",
        languages: "Языки",
        lastTestedDate: "Последняя сдача",
        manufacturer: "Производитель",
        medicationType: "Тип препарата",
        modalityType: "Модальность",
        name: "Название",
        notes: "Заметки",
        onsetDate: "Начало",
        originalText: "Исходный текст",
        position: "Положение",
        pregnancyStatus: "Беременность",
        purpose: "Назначение",
        radiologistName: "Рентгенолог",
        reaction: "Реакция",
        resolvedDate: "Разрешилось",
        restingHeartRate: "Пульс покоя",
        rhFactor: "Резус-фактор",
        scheduleNotes: "Заметки к расписанию",
        severity: "Тяжесть",
        sex: "Пол",
        sleepHours: "Сон (часов)",
        sleepQuality: "Качество сна",
        smoking: "Курение",
        specialty: "Специальность",
        startDate: "Дата начала",
        status: "Статус",
        steps: "Шаги",
        stressLevel: "Уровень стресса",
        summary: "Сводка",
        symptomName: "Симптом",
        systolic: "Систолическое",
        tags: "Теги",
        taken: "Принято",
        time: "Время",
        times: "Время приёма",
        title: "Заголовок",
        trainingIntensity: "Интенсивность тренировки",
        trainingMinutes: "Тренировка (минут)",
        vaccineName: "Вакцина",
        weightKg: "Вес",
        text: "Текст",
        value: "Значение",
        unit: "Единица",
      },
      entity: {
        medication: "Лекарство или БАД",
        medication_intake: "Приём лекарства",
        diagnosis: "Диагноз",
        allergy: "Аллергия",
        symptom: "Симптом",
        weight: "Вес",
        blood_pressure: "Давление",
        lifestyle: "Образ жизни",
        health_note: "Заметка о здоровье",
        visit: "Визит",
        imaging: "Исследование",
        vaccine: "Прививка",
        retest_schedule: "Напоминание об анализе",
        profile: "Профиль",
      },
    },
  },
  aiInterpretation: {
    title: "AI-интерпретация тренда",
    prompt: "Краткое объяснение этого тренда с учётом ваших лекарств и диапазонов.",
    button: "Объяснить тренд",
    loading: "Анализирую…",
    retry: "Повторить",
    regenerate: "Сгенерировать заново",
    cached: "Сохранено с прошлого запуска",
    errorAuth: "Ключ API отклонён. Проверьте настройки AI.",
    errorGeneric: "Не удалось получить ответ. Попробуйте ещё раз.",
  },

  // Журнал приёма лекарств
  adherence: {
    title: "Приём",
    markTaken: "Отметить приём сегодня",
    markSkipped: "Пропущено сегодня",
    loggedToday: "Отмечено на сегодня",
    taken: "Принято",
    skipped: "Пропущено",
    streak: "Серия: {{count}} дн.",
    windowSummary: "{{pct}}% за последние {{days}} дн.",
    noLogs: "Пока нет отметок о приёме.",
    delete: "Удалить запись",
  },

  // Theme
  theme: {
    lightMode: "Светлая тема",
    darkMode: "Тёмная тема",
    toggleTheme: "Переключить тему",
  },

  // Common actions
  dialog: {
    discardTitle: "Отменить изменения?",
    discardBody: "Есть несохранённые изменения. Если закрыть сейчас, они будут потеряны.",
    discard: "Отменить",
    keepEditing: "Продолжить редактирование",
  },
  errors: {
    pageTitle: "Не удалось загрузить страницу",
    pageBody:
      "При загрузке данных произошла ошибка. Подробности записаны в файл журнала (Настройки → Открыть журнал).",
    retry: "Повторить",
    actionFailed: "Не удалось выполнить действие. Попробуйте ещё раз.",
    prescriptionHasMedications:
      "Назначение не удалено: к нему всё ещё привязаны лекарства. Подтвердите, что их можно отвязать, и попробуйте снова.",
  },
  common: {
    save: "Сохранить",
    cancel: "Отменить",
    delete: "Удалить",
    edit: "Редактировать",
    add: "Добавить",
    back: "Назад",
    continue: "Продолжить",
    skipForNow: "Пропустить",
    dismiss: "Скрыть",
    export: "Экспорт",
    import: "Импорт",
    remove: "Удалить",
    close: "Закрыть",
    copy: "Копировать",
    select: "Выбрать",
    undo: "Отменить",
    kg: "кг",
    resume: "Возобновить",
    expand: "Развернуть",
    collapse: "Свернуть",
    selectModel: "Выберите модель…",
    noMatches: "Нет совпадений",
    useCustomValue: "Использовать «{{value}}»",
    search: "Поиск…",
    saveChanges: "Сохранить изменения",
    addVisit: "Добавить визит",
    validation: {
      endBeforeStart: "Дата окончания не может быть раньше даты начала.",
      resolvedBeforeStart: "Дата разрешения не может быть раньше даты диагноза.",
      expiryBeforeDose: "Срок действия не может быть раньше даты постановки.",
      futureBirthDate: "Дата рождения не может быть в будущем.",
      bpOrder: "Систолическое должно быть выше диастолического — проверьте значения.",
    },
  },

  poweredBy: {
    prefix: "Powered by",
  },

  // Common field labels
  placeholders: {
    diagnosisName: "например, субклинический гипотиреоз",
    specialty: "эндокринолог…",
    drugExample: "например, витамин D3",
    doseUnit: "МЕ",
    frequency: "ежедневно",
    prescriptionNotes: "например, 3 месяца, потом пересдать 25-OH",
    labName: "например, Инвитро",
    allergen: "например, пенициллин, арахис",
    reaction: "например, крапивница, отёк, анафилаксия",
    notes: "Дополнительные заметки",
    vaccineName: "например, COVID-19, грипп",
    manufacturer: "например, Pfizer",
    lotNumber: "например, EK5730",
    country: "например, Германия",
    clinic: "например, городская клиника",
    medicationName: "например, магния глицинат",
    medicationDoseUnit: "мг / МЕ / г",
    schedule: "с едой, перед сном, 5 через 2…",
    purpose: "качество сна, ферритин…",
  },

  fields: {
    date: "Дата",
    name: "Название",
    city: "Город",
    country: "Страна",
    type: "Тип",
    sampleTypes: "Типы образцов",
    cost: "Стоимость",
    costHint: "Сколько заплатили за весь чек-ап (USD).",
    notes: "Заметки",
    unit: "Единица",
    value: "Значение",
  },

  // Common categories
  categories: {
    custom: "Пользовательский",
  },

  // Common status options
  status: {
    active: "Активно",
    remission: "Ремиссия",
    resolved: "Решено",
  },

  // Common type options
  types: {
    blood: "Кровь",
    urine: "Моча",
    stool: "Кал",
    other: "Другое",
    supplement: "Добавка",
    drug: "Лекарство",
  },

  // Common frequency options
  frequency: {
    daily: "Ежедневно",
    twiceDaily: "2× в день",
    thriceDaily: "3× в день",
    weekly: "Еженедельно",
    asNeeded: "По необходимости",
    custom: "Индивидуально",
  },

  // Тосты-уведомления (подтверждение успешных действий)
  toasts: {
    added: "Добавлено: {{name}}",
    updated: "Обновлено: {{name}}",
    deleted: "Удалено: {{name}}",
    saved: "Сохранено",
    profileSaved: "Профиль сохранён",
    apiKeyRemoved: "API-ключ удалён",
    medStopped: "Приём прекращён: {{name}}",
    medResumed: "Приём возобновлён: {{name}}",
    dxRemission: "Переведено в ремиссию: {{name}}",
    dxResolved: "Отмечено разрешённым: {{name}}",
    allergyResolved: "Отмечено разрешённым: {{name}}",
    panelSaved: "Панель анализов сохранена",
    importSaved: "Импорт сохранён",
    // Вторая строка Undo-тоста: чего «Отменить» НЕ вернёт.
    undoCaveat: {
      file: "«Отменить» не вернёт файл-источник.",
      links: "«Отменить» не вернёт связь с назначениями.",
      log: "«Отменить» не вернёт историю приёма.",
    },
  },

  // Подтверждения удаления. Каждый вопрос называет конкретную запись, а если
  // удаление тянет за собой дочерние строки, честно перечисляет их.
  confirm: {
    unnamedTarget: "запись без названия",
    irreversible: "Вернуть её потом не получится.",
    undoHint: "Сразу после удаления можно нажать «Отменить» в уведомлении.",
    undoPartial: "Сразу после удаления можно нажать «Отменить» в уведомлении, но вернётся не всё:",
    deleteAndDetach: "Удалить и отвязать",
    // Что не вернёт частичная отмена. Каждая строка называет потерю и то, что
    // «Отменить» сделает вместо этого, чтобы решение было взвешенным.
    undoCaveat: {
      file: "Прикреплённый файл стирается с диска насовсем; «Отменить» вернёт запись уже без него.",
      links:
        "«Отменить» вернёт визит и снова привяжет к нему диагнозы, симптомы и снимки, но назначения останутся без связи.",
      log: "История приёма стирается насовсем; «Отменить» вернёт лекарство с пустым журналом.",
    },
    period: {
      since: "с {{date}}",
      range: "{{from}} — {{to}}",
    },
    cascade: {
      chatMessage: {
        one: "Вместе с ним удалится {{n}} сообщение.",
        few: "Вместе с ним удалятся {{n}} сообщения.",
        many: "Вместе с ним удалится {{n}} сообщений.",
      },
      chatDraft: {
        one: "Пропадёт {{n}} несохранённый черновик изменений.",
        few: "Пропадут {{n}} несохранённых черновика изменений.",
        many: "Пропадут {{n}} несохранённых черновиков изменений.",
      },
      medicationLog: {
        one: "Вместе с ним удалится {{n}} запись о приёме.",
        few: "Вместе с ним удалятся {{n}} записи о приёме.",
        many: "Вместе с ним удалится {{n}} записей о приёме.",
      },
      labResult: {
        one: "Вместе с ней удалится {{n}} результат по биомаркеру.",
        few: "Вместе с ней удалятся {{n}} результата по биомаркерам.",
        many: "Вместе с ней удалится {{n}} результатов по биомаркерам.",
      },
      labFinding: {
        one: "Вместе с ней удалится {{n}} нераспознанная строка.",
        few: "Вместе с ней удалятся {{n}} нераспознанные строки.",
        many: "Вместе с ней удалится {{n}} нераспознанных строк.",
      },
    },
    notes: {
      attachedFile: "Прикреплённый файл источника тоже удалится с диска.",
      visitLinks:
        "Диагнозы, назначения, симптомы и снимки этого визита останутся в записях, но потеряют связь с ним.",
      prescriptionDetach: {
        one: "По нему ведётся {{n}} лекарство. Оно останется в списке, но потеряет связь с этим назначением:",
        few: "По нему ведутся {{n}} лекарства. Они останутся в списке, но потеряют связь с этим назначением:",
        many: "По нему ведётся {{n}} лекарств. Они останутся в списке, но потеряют связь с этим назначением:",
      },
      medicationItem: "{{name}} — {{period}}",
      allergyOnEmergencyCard: "Аллерген пропадёт из карты экстренной помощи и проверок лекарств.",
    },
    delete: {
      vaccine: {
        title: "Удалить прививку «{{target}}»?",
        body: "Доза исчезнет из истории прививок и календаря.",
      },
      allergy: {
        title: "Удалить аллергию «{{target}}»?",
        body: "Аллерген перестанет проверяться при назначении лекарств.",
      },
      diagnosis: {
        title: "Удалить диагноз «{{target}}»?",
        body: "Даты, статус и заметки уйдут вместе с ним.",
      },
      medication: {
        title: "Удалить лекарство «{{target}}»?",
        body: "Пропадёт период приёма и всё, что по нему отмечено.",
      },
      medicationLog: {
        title: "Удалить отметку о приёме за {{target}}?",
        body: "Приверженность пересчитается без этого дня.",
      },
      prescription: {
        title: "Удалить назначение «{{target}}»?",
        body: "Оно пропадёт из карточки визита.",
      },
      finding: {
        title: "Удалить строку «{{target}}»?",
        body: "Нераспознанная строка исчезнет из панели.",
      },
      weight: {
        title: "Удалить запись веса за {{target}}?",
        body: "Точка пропадёт с графика веса.",
      },
      bp: {
        title: "Удалить измерение давления за {{target}}?",
        body: "Замер пропадёт с графика и из статистики давления.",
      },
      symptom: {
        title: "Удалить запись о симптоме «{{target}}»?",
        body: "Эпизод исчезнет из дневника симптомов и с графика.",
      },
      lifestyle: {
        title: "Удалить день {{target}}?",
        body: "Сон, тренировки, стресс и энергия за этот день пропадут.",
      },
      note: {
        title: "Удалить заметку «{{target}}»?",
        body: "Текст и всё, что из него разобрано, будет удалено.",
      },
      imaging: {
        title: "Удалить исследование «{{target}}»?",
        body: "Само исследование, находки и заключение пропадут.",
      },
      retest: {
        title: "Удалить напоминание о пересдаче «{{target}}»?",
        body: "Оно перестанет появляться в уведомлениях.",
      },
      visit: {
        title: "Удалить визит {{target}}?",
        body: "Приём исчезнет из истории.",
      },
      labPanel: {
        title: "Удалить панель {{target}}?",
        body: "Панель и всё, что в ней измерено, будет удалено.",
      },
      chatThread: {
        title: "Удалить чат «{{target}}»?",
        body: "Переписка и всё, что ассистент в ней искал, исчезнут. Записи, которые вы сохранили из этого чата в карту, останутся.",
      },
      apiKey: {
        title: "Удалить API-ключ для {{target}}?",
        body: "Ключ сотрётся из системного хранилища, и ИИ-функции не заработают, пока вы не сохраните новый.",
      },
    },
  },

  // Settings
  settings: {
    sections: {
      title: "Разделы в меню",
      description: "Выберите, какие разделы показывать в боковом меню.",
    },
    dashboard: {
      title: "Главный экран",
      description:
        "Выберите, какие карточки показывать на главном экране. Скрытые карточки остаются доступны из меню — меняется только дашборд.",
      widgets: {
        safetyBanner: {
          label: "Баннер критических аллергий",
          desc: "Красная полоса с предупреждением о тяжёлых и опасных для жизни аллергиях.",
        },
        verdict: {
          label: "Сводный статус",
          desc: "Строка «всё спокойно» / «нужно внимание».",
        },
        stats: {
          label: "Краткая статистика",
          desc: "Анализы, показатели вне нормы, активные препараты и последний забор.",
        },
        attention: {
          label: "Требует внимания",
          desc: "Сводка по биомаркерам, диагнозам, препаратам и прививкам.",
        },
        review: {
          label: "Очередь на проверку",
          desc: "Импортированные анализы, которые ждут вашего подтверждения.",
        },
        changes: {
          label: "Заметные изменения",
          desc: "Что изменилось с прошлой панели анализов.",
        },
        activity: {
          label: "Недавняя активность",
          desc: "Последние записи: анализы, препараты, диагнозы и другое.",
        },
      },
      safetyConfirm: {
        title: "Скрыть баннер критических аллергий?",
        body: "Этот баннер сразу предупреждает о тяжёлых, опасных для жизни аллергиях. Он останется на карте экстренной помощи, но больше не будет показываться на главном экране.",
        confirm: "Скрыть",
      },
    },
    reset: {
      title: "Сбросить персонализацию",
      description:
        "Вернуть все разделы меню и карточки дашборда, включить все уведомления и восстановить скрытые напоминания и подсказки. Ваши медицинские данные не затрагиваются.",
      action: "Сбросить персонализацию",
      done: "Персонализация сброшена к настройкам по умолчанию.",
      confirm: {
        title: "Сбросить персонализацию?",
        body: "Разделы меню, карточки дашборда, настройки уведомлений и скрытые напоминания и подсказки вернутся к значениям по умолчанию. Это не затронет ваши записи.",
      },
    },
    appearance: {
      title: "Внешний вид",
      description: "Язык и тема оформления.",
      language: "Язык",
      theme: "Тема",
      light: "Светлая",
      dark: "Тёмная",
      system: "Системная",
    },
    updates: {
      title: "Обновления",
      description: "Установленная версия: v{{version}}",
      loadingVersion: "Определяем установленную версию…",
      checking: "Проверяем GitHub Releases…",
      check: "Проверить снова",
      available: "Доступна Soma v{{version}}.",
      current: "У вас актуальная версия. Последний релиз: v{{version}}.",
      noReleases: "Опубликованных релизов пока нет.",
      error: "Не удалось проверить GitHub Releases. Проверьте соединение и повторите попытку.",
      newVersion: "Новая версия",
      openRelease: "Открыть v{{version}}",
      allReleases: "Все релизы",
      manual: "Обновления скачиваются с GitHub и устанавливаются вручную.",
    },
    logs: {
      title: "Логи",
      description: "Журнал приложения с ошибками и диагностикой. Ротация при 10 МБ.",
      open: "Открыть логи",
      error: "Не удалось открыть файл логов.",
    },
    mcp: {
      title: "Доступ для ИИ-ассистента (MCP)",
      description:
        "Позвольте ИИ-ассистенту на этом Mac читать и обновлять данные Soma через Model Context Protocol. Сервер работает локально и читает soma.db напрямую — ничего не покидает устройство, токен не нужен.",
      intro:
        "Выберите ассистента, скопируйте конфиг в указанный файл и перезапустите ассистента. Инструменты Soma (анализы, лекарства, тренды…) появятся автоматически.",
      serverPath: "Бинарь сервера",
      notBuilt:
        "Бинарь MCP-сервера ещё не собран. Выполните `pnpm mcp:sidecar` (в dev) или пересоберите приложение и откройте этот экран заново.",
      pathLabel: "Путь",
      whereToAdd: "Добавьте в",
      cliShortcut: "Или выполните в терминале:",
      copy: "Скопировать",
      copied: "Скопировано",
      copyPath: "Скопировать путь",
      installAllDetected: "Установить во все найденные",
      install: "Установить",
      update: "Обновить",
      installed: "Установлено",
      reinstall: "Переустановить",
      detected: "найден",
      notFound: "не найден",
      installedToast: "Записано в {{path}} — перезапустите {{client}}, чтобы загрузить Soma.",
      installErr: "Не удалось записать конфиг: {{msg}}",
      manualShow: "Ручная настройка (скопировать конфиг самому)",
      manualHide: "Скрыть ручную настройку",
      writesTitle: "Разрешить запись",
      writesDesc:
        "Позволяет ассистенту не только читать, но и добавлять записи (лекарства, анализы, симптомы…). Применяется ко всем ассистентам ниже и вступает в силу после их перезапуска.",
      readWrite: "чтение + запись",
      readOnly: "только чтение",
      writesConfirm: {
        title: "Разрешить ассистенту записывать данные о здоровье?",
        body: "Если включить, любой подключённый выше ассистент сможет добавлять и изменять записи в Soma — лекарства, диагнозы, прививки, заметки и другое — а не только читать их. В любой момент можно выключить на этом же экране.",
        confirm: "Разрешить запись",
      },
    },
    title: "Настройки",
    description:
      "Провайдеры ИИ, профиль и экспорт данных. Всё остаётся на этом устройстве, если вы не решите иначе.",
    tabs: {
      general: "Основные",
      ai: "ИИ",
      profile: "Профиль",
      data: "Данные",
    },
    dangerZone: "Опасная зона",

    // Language section
    language: {
      title: "Язык",
      description: "Выберите предпочитаемый язык",
      english: "English",
      russian: "Русский",
    },

    // Profile section
    profile: {
      title: "Профиль",
      description:
        "Используется для референсных диапазонов биомаркеров с учётом пола и возраста. Пока один профиль; совместное использование нескольких профилей появится позже.",
      saveProfile: "Сохранить профиль",
      saved: "Сохранено ✓",
    },

    // AI Settings
    ai: {
      title: "ИИ-анализ",
      description:
        "По умолчанию отключён. Используйте свой API-ключ — он хранится в системной связке ключей, никогда в базе данных или конфигурационных файлах. Предлагаются только мультимодальные модели (зрение + PDF), поскольку пайплайн импорта читает фотографии и PDF.",
      provider: "Провайдер",
      aiDisabled: "ИИ отключён",
      model: "Модель (только мультимодальные)",
      customModel:
        "Пользовательский ID модели (опционально, переопределяет список — должна поддерживать зрение + PDF)",
      customModelPlaceholder: "например, более новый ID модели, которого ещё нет в реестре",
      apiKeyFor: "API-ключ для",
      checking: "проверяем…",
      storedInKeychain: "сохранён в связке ключей",
      notSet: "не установлен",
      saveKey: "Сохранить ключ",
      testKey: "Тестировать ключ",
      keyWorks: "Ключ работает — модель ответила.",
      getApiKey: "Создать API-ключ",
      recommendTitle: "Какую модель выбрать?",
      recommendBody:
        "Лучшая цена: Google Gemini (тариф Flash — есть бесплатный тир и читает PDF напрямую). Лучшая точность: Anthropic Claude (Sonnet). Один ключ на много моделей: OpenRouter. Совет: Gemini и Claude читают PDF нативно; с OpenAI/OpenRouter надёжнее всего чёткие фото.",
      disclaimer:
        "Вывод ИИ никогда не является медицинской консультацией. Документы, которые вы импортируете, отправляются выбранному провайдеру только когда вы явно запускаете действие ИИ.",
      aiDisclaimer:
        "Контент создан ИИ. Не является медицинской консультацией — всегда обращайтесь к квалифицированному врачу.",
    },

    // Backup section
    backup: {
      title: "Облачное резервирование",
      // Note: BackupCard has its own complex content, leaving basic structure
    },

    // Export section
    export: {
      title: "Экспорт данных",
      description:
        "Все записи медкарты в JSON или CSV — данные ваши, без привязки к приложению. Прикреплённые документы (PDF, фото) попадают в выгрузку только именами файлов; чтобы перенести их, используйте зашифрованный бэкап.",
      exportAll: "Экспорт всех данных (JSON)",
      exportLabs: "Результаты анализов (CSV)",
      exporting: "Экспортируем…",
      error: "Не удалось экспортировать: {{message}}",
    },
  },

  // Profile form fields
  profile: {
    fields: {
      name: "Имя",
      dateOfBirth: "Дата рождения",
      biologicalSex: "Биологический пол",
      units: "Единицы измерения",
      height: "Рост",
      weight: "Вес",
      bloodGroup: "Группа крови",
      rhFactor: "Резус-фактор",
      ethnicity: "Этническая принадлежность",
      targetWeight: "Целевой вес",
      activityLevel: "Уровень активности",
      smoking: "Курение",
      alcohol: "Алкоголь",
      chronicConditions: "Хронические заболевания",
    },
    options: {
      male: "Мужской",
      female: "Женский",
      otherIntersex: "Другой / интерсекс",
      metricSystem: "Метрическая (см, кг)",
      imperialSystem: "Английская (фут/дюйм, фунт)",
      positiveRh: "Положительный (+)",
      negativeRh: "Отрицательный (−)",
      sedentary: "Малоподвижный",
      lightlyActive: "Слабоактивный",
      moderatelyActive: "Умеренно активный",
      active: "Активный",
      veryActive: "Очень активный",
      never: "Никогда",
      former: "Раньше",
      current: "Сейчас",
      none: "Не употребляю",
      occasional: "Иногда",
      moderate: "Умеренно",
      heavy: "Часто",
      bloodO: "O (I)",
      bloodA: "A (II)",
      bloodB: "B (III)",
      bloodAB: "AB (IV)",
    },
    descriptions: {
      sedentary: "Сидячая работа, без регулярных тренировок",
      lightlyActive: "Прогулки или лёгкие нагрузки 1–2 раза в неделю",
      moderatelyActive: "Тренировки 2–4 раза в неделю",
      active: "Интенсивные тренировки 4–6 раз в неделю",
      veryActive: "Ежедневные интенсивные тренировки или физический труд",
      smokingFormer: "Бросил(а) — влияет на некоторые диапазоны годами",
      alcoholOccasional: "Несколько порций в месяц",
      alcoholModerate: "До ~7 порций в неделю",
    },
    hints: {
      ethnicity: "Влияет на некоторые референсные диапазоны (например, СКФ, показатели крови).",
      targetWeight: "Необязательная цель — отображается линией на графике веса в «Журнале».",
    },
    ethnicity: {
      white: "Европеоидная",
      black: "Негроидная / африканская",
      eastAsian: "Восточноазиатская",
      southAsian: "Южноазиатская",
      southeastAsian: "Юго-восточноазиатская",
      centralAsian: "Центральноазиатская",
      mena: "Ближневосточная / североафриканская",
      hispanic: "Латиноамериканская",
      nativeAmerican: "Коренные народы Америки",
      pacificIslander: "Народы Океании",
      ashkenaziJewish: "Евреи-ашкеназы",
      caribbean: "Карибская",
      mixed: "Смешанная",
      other: "Другая",
    },
    placeholders: {
      ft: "ft",
      in: "in",
      cm: "cm",
      lb: "lb",
      kg: "kg",
      ethnicity: "напр. восточноазиатская, чёрная, белая…",
      optional: "опционально",
      chronicConditions: "напр. гипотиреоз, диабет 2 типа… (свободный текст)",
    },
  },

  // Onboarding
  onboarding: {
    welcomeTitle: "Добро пожаловать в Soma",
    welcomeDescription:
      "Ваша медицинская карта на ваших условиях — анализы, лекарства и визиты в одной приватной временной шкале. Минута настройки позволит Soma оценивать ваши результаты по тому, что считается нормой именно для вас.",
    privacyNote:
      "Всё, что вы вводите, остаётся на этом устройстве — никаких аккаунтов, облака или отслеживания.",
    getStarted: "Начать",
    language: "Язык",

    aboutYouTitle: "О вас",
    aboutYouDescription:
      "Soma использует ваш пол и возраст, чтобы понимать, какие результаты для вас в норме — только поэтому эти поля обязательны.",

    fineTuningTitle: "Тонкая настройка",
    fineTuningDescription:
      "Необязательно, но стоит того: это уточняет некоторые референсные диапазоны и помогает отслеживать цели. Пропускайте что угодно — всё можно изменить позже в настройках.",

    allSetTitle: "Всё готово",
    allSetTitleWithName: "Всё готово, {{name}}",
    allSetDescription:
      "Ваш профиль сохранён локально. Импортируйте отчёт о лабораторных исследованиях, записывайте что принимаете, и Soma начнёт соединять точки.",
    tip: "Совет: наложите любое лекарство на график биомаркера, чтобы увидеть, действительно ли то, что вы принимаете, влияет на показатели.",
    openDashboard: "Открыть главную",
    disclaimer:
      "Soma организует ваши записи — это не медицинское устройство и не даёт медицинских советов.",
    restore: "У меня уже есть бэкап",
    interestsTitle: "Что вы хотите отслеживать?",
    interestsDescription:
      "Выберите интересующие разделы. Остальные не будут показываться в меню — включить их можно в любой момент в настройках.",
    interestsHint: "Анализы, биомаркеры и шкала всегда включены.",
    groups: {
      meds: { label: "Лекарства и добавки", desc: "Дозы, расписание, периоды приёма" },
      conditions: { label: "Визиты и диагнозы", desc: "Визиты к врачу, диагнозы, рецепты" },
      allergies: { label: "Аллергии и неотложная карта", desc: "Записи об аллергиях и карта" },
      vaccines: { label: "Прививки", desc: "Записи, серии и советы для поездок" },
      imaging: { label: "Снимки", desc: "Рентген, КТ, МРТ, УЗИ" },
      vitals: { label: "Дневник и образ жизни", desc: "Вес, давление, сон, стресс" },
      ai: { label: "Soma AI и импорт", desc: "Чат и импорт фото/PDF" },
    },
  },

  gettingStarted: {
    title: "С чего начать",
    description: "Несколько шагов, чтобы Soma стала полезной.",
    importLab: "Импортируйте или добавьте первый анализ",
    addMed: "Добавьте лекарство или добавку",
    enableAi: "Включите AI-импорт и ассистента (по желанию)",
    cta: "Открыть",
    skip: "Пропустить настройку",
  },

  hints: {
    overlayTitle: "Совет: наложите лекарства на график",
    overlayBody:
      "Включите любое лекарство ниже, чтобы увидеть период приёма полосой поверх тренда — и понять, влияет ли то, что вы принимаете, на показатель.",
  },

  // Page headers (will be used by PageHeader component)
  pages: {
    dashboard: {
      title: "Главная",
      description: "Ваше здоровье с первого взгляда — локально, приватно, ваше.",
    },
    timeline: {
      title: "Временная шкала",
      description: "Все события здоровья в хронологическом порядке",
    },
    biomarkers: {
      title: "Биомаркеры",
      description: "Отслеживание лабораторных показателей во времени",
    },
    labs: {
      title: "Результаты анализов",
      description: "Импорт и управление лабораторными отчётами",
    },
    medications: {
      title: "Лекарства",
      description: "Что вы принимаете и когда",
    },
    visits: {
      title: "Медицинские визиты",
      description: "Приёмы врачей и консультации",
    },
    diagnoses: {
      title: "Диагнозы",
      description: "Медицинские состояния и диагнозы",
    },
  },

  // Loading states
  loading: {
    openingDatabase: "Открываем локальную базу данных…",
  },

  // Error states
  error: {
    databaseFailed: "Не удалось открыть локальную базу данных",
  },

  // Сравнение двух панелей анализов
  labCompare: {
    title: "Сравнить даты анализов",
    description: "Две панели рядом и что изменилось между ними.",
    button: "Сравнить",
    panelA: "Первая панель",
    panelB: "Вторая панель",
    selectPrompt: "Выберите две панели для сравнения.",
    samePanel: "Выберите две разные панели.",
    needTwoPanels: "Нужно минимум две панели анализов для сравнения.",
    columns: {
      biomarker: "Биомаркер",
      change: "Изменение",
      status: "Статус",
    },
  },

  // Labs page
  labs: {
    title: "Результаты анализов",
    description: "Каждый забор крови и анализ мочи, введённые вручную или импортированные ИИ.",
    aiImport: "ИИ-импорт",
    newPanel: "Новая панель",
    emptyTitle: "Пока нет панелей анализов",
    emptyDescription:
      "Введите результаты вручную или импортируйте PDF/фото лабораторного отчёта с помощью ИИ.",
    addFirstPanel: "Добавить первую панель",
    findingsTitle: "Дополнительные находки",
    findingsDescription:
      "Результаты, сохранённые без биомаркера (несопоставленные или качественные), сгруппированные по всем панелям. Нажмите на строку, чтобы открыть панель.",
    tableColumns: {
      date: "Дата",
      lab: "Лаборатория",
      location: "Местоположение",
      type: "Тип",
      results: "Результаты",
      outOfRange: "Вне диапазона",
      source: "Источник",
      finding: "Показатель / исходная метка",
    },
    importSource: {
      ai: "ИИ",
      manual: "вручную",
    },
  },

  // Timeline page
  timeline: {
    title: "Временная шкала",
    description: "Анализы рядом с лекарствами, весом и показателями — одна настраиваемая шкала.",
    layersTitle: "Слои",
    shiftHint:
      "Подсвеченные точки анализов — панели, где биомаркер сильно изменился по сравнению с предыдущей.",
    shiftCount: "Заметных изменений к прошлому: {{count}}",
    medsMore: "+{{count}} ещё лекарств",
    medsLess: "Свернуть лекарства",
    scrollHint: "Прокручивайте горизонтально, чтобы посмотреть всю историю.",
    scrollAreaLabel: "Прокручиваемая временная шкала здоровья",
    legend: {
      title: "Легенда",
      outOfRange: "Анализ вне диапазона",
      shiftWatch: "Заметный сдвиг",
      shiftAlert: "Выход за диапазон / критично",
      bpNormal: "Давление в норме",
      bpStage1: "Давление повышено",
      bpStage2: "Давление 2 ст.",
      bpCrisis: "Гипертонический криз",
      today: "Сегодня",
    },
    layers: {
      lab_panel: "Анализы",
      medication: "Лекарства",
      weight: "Вес",
      bp: "Давление",
      symptom: "Симптомы",
      visit: "Визиты",
      diagnosis: "Диагнозы",
      vaccine: "Прививки",
      allergy: "Аллергии",
      imaging: "Снимки",
      health_note: "Заметка о здоровье",
    },
    ranges: {
      sixMonths: "6М",
      oneYear: "1Г",
      twoYears: "2Г",
      all: "Всё",
    },
    emptyTitle: "Временная шкала пуста",
    emptyDescription:
      "События появятся здесь по мере добавления анализов, лекарств, визитов и диагнозов.",
    allEvents: "Все события",
    eventKinds: {
      labs: "анализы",
      medication: "лекарство",
      diagnosis: "диагноз",
      visit: "визит",
      allergy: "аллергия",
      vaccine: "прививка",
      symptom: "симптом",
      imaging: "снимок",
    },
    outOfRange: "вне диапазона",
    now: "сейчас",
    symptomThreshold: "Скрыто записей о лёгких симптомах (тяжесть < 6): {{count}}.",
    showAllSymptoms: "Показать все",
    hideMinorSymptoms: "Скрыть лёгкие симптомы",
  },

  // Lab panel detail page
  labPanelDetail: {
    backToLabs: "Результаты анализов",
    aiImported: "Импортировано ИИ",
    allInRange: "всё в диапазоне",
    outOfRange: "вне диапазона",
    deletePanel: "Удалить панель",
    deletedToast: "Панель удалена.",
    findingsTitle: "Дополнительные находки",
    findingsDescription:
      "Результаты, сохранённые без биомаркера (несопоставленные или качественные). Редактируются, в трендах не участвуют.",
    editFindingTitle: "Редактировать находку",
    editResultTitle: "Редактировать результат",
    editResultDescription:
      "Исправьте значение или его единицу. Нормализация и флаги пересчитываются; при нераспознанной единице значение сохраняется как есть, без флагов.",
    resultUpdatedToast: "Результат обновлён.",
    editPanel: "Редактировать панель",
    editPanelTitle: "Редактировать панель",
    editPanelDescription: "Исправьте дату, лабораторию и контекст панели. Результаты не меняются.",
    panelUpdatedToast: "Панель обновлена.",
    panelNotFound: "Панель не найдена",
    noResults: "В этой панели нет числовых результатов.",
    findingsOnly: "В панели только качественные находки — тренды не строятся.",
    tableColumns: {
      biomarker: "Биомаркер",
      value: "Значение",
      change: "Δ к прошлому",
      normalized: "Нормализованное",
      reference: "Референс",
      status: "Статус",
      sourceLabel: "Исходная метка",
    },
  },

  // Корреляция между панелями («что изменилось с прошлого раза»)
  insights: {
    title: "Заметные изменения",
    sinceLast: "Сравнение с вашим предыдущим результатом по каждому биомаркеру.",
    dashboardTitle: "Что изменилось",
    dashboardSince: "Заметные сдвиги в последней панели ({{date}}) по сравнению с прошлой.",
    baseline:
      "Это первые значения по этим биомаркерам — будущие импорты будут сравниваться с ними.",
    allStable: "С прошлого раза заметных изменений нет — значения стабильны.",
    since: "с {{date}}",
    unitChanged: "Единицы отличаются от прошлого замера — изменение несравнимо.",
    unitChangedShort: "ед. ≠",
    howTitle: "Как это определяется?",
    howBody:
      "Изменение отмечается, если оно выходит за референсный диапазон, достигает критического уровня, превышает ~40% ширины диапазона или меняется на ~50%+ относительно прошлого раза. Зелёный — сдвиг в сторону нормы, красный — от неё.",
    trajectory: {
      improved: "улучшение",
      worsened: "ухудшение",
      neutral: "изменение",
    },
    reason: {
      became_out_of_range: "вышло за диапазон",
      worsened_critical: "достигло критического уровня",
      back_in_range: "вернулось в диапазон",
      large_move: "резко изменилось",
      moved_within_range: "сдвинулось (в пределах нормы)",
    },
  },

  // Biomarkers page
  biomarkers: {
    title: "Биомаркеры",
    description:
      "Справочный словарь с нормальными и оптимальными диапазонами. Нажмите на биомаркер, чтобы увидеть тренд.",
    customBiomarker: "Пользовательский биомаркер",
    searchPlaceholder: "Поиск по названию или псевдониму…",
    withDataOnly: "Только с данными",
    emptySearchTitle: "Ничего не найдено",
    emptySearchDescription: "Попробуйте другой поисковый запрос.",
    custom: "пользовательский",
    noData: "Нет данных",
    optimal: "оптимально",
    inRange: "в норме",
    createDialog: {
      title: "Новый пользовательский биомаркер",
      description: "Для аналитов, отсутствующих во встроенном словаре.",
      nameLabel: "Название",
      namePlaceholder: "например, Индекс омега-3",
      categoryLabel: "Категория",
      unitLabel: "Единица",
      unitPlaceholder: "например, %",
      refLowLabel: "Нижний реф.",
      refHighLabel: "Верхний реф.",
      noRangeWarning:
        "Без референсного диапазона этот биомаркер никогда не будет помечен как вне нормы.",
      directionLabel: "Направление",
      directionOptions: {
        range: "В диапазоне",
        higherBetter: "Выше лучше",
        lowerBetter: "Ниже лучше",
      },
      aliasesLabel: "Псевдонимы (через запятую, любой язык)",
      aliasesPlaceholder: "synonym 1, синоним 2",
      create: "Создать",
    },
    editDialog: {
      action: "Изменить диапазоны",
      title: "Редактировать запись словаря",
      refLowLabel: "Нижний реф.",
      refHighLabel: "Верхний реф.",
      optimalLowLabel: "Нижний оптим.",
      optimalHighLabel: "Верхний оптим.",
      aliasesLabel: "Псевдонимы (через запятую или с новой строки, любой язык)",
      aliasesPlaceholder: "synonym 1, синоним 2",
      refOrderError: "Нижний референс не может быть больше верхнего.",
      optimalOrderError: "Нижний оптимум не может быть больше верхнего.",
    },
  },

  // Medications page
  medications: {
    title: "Лекарства и добавки",
    description:
      "Что вы принимаете, в какой дозе и с какого времени — можно накладывать на любой тренд биомаркеров.",
    emptyTitle: "Пока ничего не отслеживается",
    emptyDescription:
      "Добавьте лекарства и добавки с дозами и периодами, чтобы сопоставить их с вашими анализами.",
    addFirst: "Добавить первый элемент",
    currentlyTaking: "Принимаю сейчас",
    past: "Прошлое",
    timeline: {
      title: "Таймлайн приёма",
      now: "сейчас",
    },
    addDialog: {
      titleAdd: "Добавить лекарство или добавку",
      titleEdit: "Редактировать лекарство",
    },
    doseUnits: {
      drops: "капли",
      tablets: "таблетки",
      capsules: "капсулы",
      sprays: "впрыски",
    },
    fields: {
      dose: "Доза",
      frequency: "Частота",
      scheduleNotesOptional: "Заметки о расписании (опционально)",
      startDate: "Дата начала",
      endDateOptional: "Дата окончания (пустая = продолжается)",
      purposeOptional: "Цель (опционально)",
    },
    actions: {
      stopToday: "Прекратить сегодня",
      resume: "Возобновить",
    },
  },

  // Visits page
  visits: {
    title: "Визиты к врачам",
    description: "Консультации в клиниках, городах и странах.",
    emptyTitle: "Пока нет визитов",
    emptyDescription: "Записывайте консультации врачей с диагнозами и назначениями.",
    addFirst: "Добавить первый визит",
    addDialog: {
      titleAdd: "Новый визит к врачу",
      titleEdit: "Редактировать визит",
    },
    fields: {
      doctor: "Врач",
      specialty: "Специальность",
      clinic: "Клиника",
      location: "Местоположение",
    },
  },

  // Diagnoses page
  diagnoses: {
    title: "Диагнозы",
    description: "Активные состояния и история разрешённых случаев.",
    emptyTitle: "Диагнозы не записаны",
    emptyDescription: "Отслеживайте состояния со статусом: активно, в ремиссии или разрешено.",
    addFirst: "Добавить первый диагноз",
    timeline: {
      title: "Таймлайн состояний",
    },
    addDialog: {
      titleAdd: "Добавить диагноз",
      titleEdit: "Редактировать диагноз",
    },
    fields: {
      diagnosis: "Диагноз",
      icd: "МКБ",
      icdCodeOptional: "Код МКБ (опционально)",
      status: "Статус",
      notesOptional: "Заметки (опционально)",
      resolvedDate: "Дата разрешения",
    },
    sections: {
      active: "Активные",
      remission: "В ремиссии",
      resolved: "Разрешённые",
    },
    actions: {
      moveToRemission: "В ремиссию",
      resolve: "Разрешить",
      confirmResolve: "Подтвердить",
    },
  },

  // Allergies page
  allergies: {
    title: "Аллергии",
    description:
      "Аллергии на лекарства, продукты питания и окружающую среду с указанием тяжести и реакции.",
    emptyTitle: "Аллергии не записаны",
    emptyDescription: "Отслеживайте аллергические реакции и непереносимость.",
    addFirst: "Добавить первую аллергию",
    addDialog: {
      titleAdd: "Добавить аллергию",
      titleEdit: "Редактировать аллергию",
    },
    fields: {
      allergen: "Аллерген",
      category: "Категория",
      severity: "Тяжесть",
      status: "Статус",
      onsetDateOptional: "Дата начала (опционально)",
      reactionOptional: "Реакция (опционально)",
      notesOptional: "Заметки (опционально)",
    },
    sections: {
      active: "Активные",
      resolved: "Разрешённые",
    },
    actions: {
      resolve: "Разрешить",
    },
    deleteAnaphylacticTooltip: "Анафилактическую аллергию нельзя удалить — сначала разрешите её.",
    drugGuardWarning: "⚠ {{allergen}} — у вас {{severity}} аллергия",
  },

  // Health notes (free-form record entries, incl. ones the assistant saved)
  healthNotes: {
    title: "Заметки о здоровье",
    description:
      "Свободный контекст, который не укладывается в структурированную запись — ваш и сохранённый Soma AI.",
    emptyTitle: "Заметок пока нет",
    emptyDescription:
      "Семейный анамнез, беспокойство, паттерн симптомов, история лечения — всё, для чего нет отдельной формы. Сюда же Soma AI сохраняет свои заметки.",
    addFirst: "Добавить первую заметку",
    searchPlaceholder: "Поиск по заметкам…",
    noMatchesTitle: "Ничего не найдено",
    noMatchesDescription: "Попробуйте другую категорию или запрос.",
    noDate: "Без даты",
    showOriginal: "Показать исходный текст",
    hideOriginal: "Скрыть исходный текст",
    dialog: {
      titleAdd: "Новая заметка",
      titleEdit: "Редактировать заметку",
    },
    fields: {
      category: "Категория",
      titleOptional: "Заголовок (необязательно)",
      text: "Заметка",
      summaryOptional: "Краткое изложение (необязательно)",
      dateOptional: "Дата (необязательно)",
      datePrecision: "Точность даты",
      dateRawOptional: "Дата словами (необязательно)",
      tagsOptional: "Теги (необязательно, через запятую)",
    },
    placeholders: {
      title: "напр. Семейный анамнез — аутоиммунное",
      text: "Своими словами",
      summary: "Одна строка, которую врач прочитает по диагонали",
      dateRaw: "напр. прошлой весной, в детстве",
      tags: "напр. семья, щитовидка",
    },
  },

  // Health note category options
  healthNoteCategory: {
    general: "Общее",
    concern: "Беспокойство",
    symptom_pattern: "Паттерн симптомов",
    treatment: "Лечение",
    history: "Анамнез",
    other: "Другое",
  },
  healthNoteCategoryDescription: {
    general: "Контекст, которому нет другого места",
    concern: "То, что вас беспокоит",
    symptom_pattern: "Закономерность во времени, а не разовый эпизод",
    treatment: "Терапия, процедура или её результат",
    history: "Личный или семейный анамнез",
    other: "Всё остальное, что стоит сохранить",
  },

  // How precisely a date is known
  datePrecision: {
    day: "Точный день",
    month: "Месяц",
    year: "Год",
    approximate: "Примерно",
    range: "Период",
    unknown: "Неизвестно",
  },

  // Allergy category options
  allergyCategory: {
    drug: "Лекарство",
    food: "Еда",
    environmental: "Окружающая среда",
    other: "Другое",
  },

  // Allergy severity options
  allergySeverity: {
    mild: "Лёгкая",
    moderate: "Умеренная",
    severe: "Тяжёлая",
    anaphylactic: "Анафилактическая",
  },

  allergySeverityDescription: {
    mild: "Дискомфорт, лечение не требуется",
    moderate: "Нужны антигистаминные или лечение",
    severe: "Сильная системная реакция",
    anaphylactic: "Угроза жизни — экстренный адреналин",
  },

  // Vaccines page
  vaccines: {
    title: "Прививки",
    description: "История вакцинации с дозами, сериями и сроками годности.",
    emptyTitle: "Прививки не записаны",
    emptyDescription: "Ведите историю своей иммунизации.",
    addFirst: "Добавить первую прививку",
    addDialog: {
      titleAdd: "Добавить запись о прививке",
      titleEdit: "Редактировать запись о прививке",
    },
    fields: {
      vaccineName: "Название вакцины",
      date: "Дата",
      doseNumber: "Доза №",
      manufacturer: "Производитель",
      batchNumber: "Серия",
      expiresOptional: "Срок годности (опционально)",
      country: "Страна",
      administeredByOptional: "Кто ввёл (опционально)",
      notesOptional: "Заметки (опционально)",
    },
    table: {
      date: "Дата",
      dose: "Доза №",
      manufacturerBatch: "Производитель / Серия",
      country: "Страна",
      administeredBy: "Кто ввёл",
      expires: "Годен до",
    },
    expired: "Истёк",
    expiry: {
      lifetime: "Пожизненная действительность — без срока (ВОЗ).",
      suggest: "Рекомендуемый срок: {{date}} — нажмите, чтобы подставить",
    },
    recordsTitle: "Записанные прививки",
    timeline: {
      title: "Таймлайн прививок",
    },
    calendar: {
      title: "Календарь прививок",
      subtitle: "Справочное расписание ВОЗ",
      description:
        "По антигенам, вне привязки к стране. Национальные календари отличаются — это базовый ориентир ВОЗ, а не медицинская рекомендация.",
      addBirthDate:
        "Укажите дату рождения в профиле, чтобы видеть персональные статусы «пора / требует действия».",
      summaryClear: "Сейчас ничего не требует действий.",
      actionable: "требует действия: {{n}}",
      due: "предстоит: {{n}}",
      doseN: "Доза {{n}}",
      next: "след. {{date}}",
      protects: "Защищает от: {{disease}}",
      status: {
        done: "Сделано",
        due: "Пора",
        overdue: "Просрочено",
        upcoming: "Предстоит",
        contextual: "По показаниям",
        not_recorded: "Не отмечено",
      },
      tiers: {
        universal: "Детская серия",
        special: "Часто добавляют",
        regional: "Региональные",
        risk: "Группы риска и поездки",
      },
      tierBlurbs: {
        universal:
          "Обычно завершается в детстве. Отметьте или импортируйте, если делали — неотмеченные не считаются просроченными.",
        special: "Часто добавляют в зависимости от страны и истории прививок.",
        regional: "Актуально для путешествий — справочно, зависит от региона поездки.",
        risk: "Актуально для путешествий — справочно, зависит от направления и контакта.",
      },
    },
  },

  // OS keychain availability (API keys + backup passphrase)
  keychain: {
    unavailableTitle: "Системное хранилище ключей недоступно",
    unavailableLinux:
      "Soma хранит API-ключи и пароль от резервных копий в Linux Secret Service — ничего секретного не пишется на диск в открытом виде. Сейчас ни одно хранилище ключей не отвечает. Установите и запустите его (например, gnome-keyring или KWallet) и нажмите «Проверить снова». Всё остальное в Soma работает и без него.",
    unavailableGeneric:
      "Soma не смогла обратиться к системному хранилищу ключей ({{backend}}). Убедитесь, что оно доступно, и нажмите «Проверить снова».",
    recheck: "Проверить снова",
    saveFailed:
      "Не удалось сохранить в системное хранилище. Смотрите уведомление выше и нажмите «Проверить снова».",
  },

  // Backup card
  backup: {
    title: "Резервные копии",
    description:
      "Зашифрованные снимки вашей базы данных, сохранённые в папку, которую ваш облачный клиент (iCloud Drive, Google Drive, Dropbox, OneDrive) уже синхронизирует. Soma никогда не загружает ничего сама и сохраняет только зашифрованные .somabk файлы там — ваши живые данные никогда не покидают это устройство.",
    setupBackups: "Настроить резервирование",
    disable: "Отключить",
    backupNow: "Создать резервную копию",
    backingUp: "Создаём резервную копию…",
    restoreFromBackup: "Восстановить из резервной копии…",
    destination: "Назначение",
    frequency: "Частота",
    lastBackup: "Последняя резервная копия",
    noBackupYet: "Резервные копии ещё не создавались.",
    rotationNote: "Старые снимки автоматически удаляются (сохраняются 12 новейших).",
    lastAttemptFailed: "Последняя попытка не удалась",
    disableNote:
      "Отключение только останавливает будущие резервные копии — существующие снимки и ваша парольная фраза сохраняются, поэтому вы можете повторно включить в любое время.",

    // Setup wizard
    setupStep: "Настройка резервирования · шаг {{step}} из 3",
    step1Description: "Где должны сохраняться зашифрованные снимки?",
    step2Description: "Защитите ваши резервные копии парольной фразой.",
    step3Description: "Как часто Soma должна создавать резервные копии?",

    // Step 1 - destination
    macosOnly: "только macOS",
    notFoundOnComputer: "Не найдено на этом компьютере",
    detected: "Обнаружено",
    chooseManually: "Выбрать папку вручную…",
    customWarning:
      "Эта папка не является одной из обнаруженных облачных папок. Резервные копии, сохранённые здесь, попадут в облако только если что-то на этом компьютере синхронизирует эту папку.",
    folderNotExist: "Эта папка не существует.",
    folderNotWritable: "Soma не может записывать в эту папку — выберите другую.",
    subfolderNote:
      "Внутри создаётся подпапка Soma Backups с README, объясняющим файлы. Туда попадают только зашифрованные снимки — никогда ваши живые данные.",

    // Step 2 - passphrase
    passphraseTitle:
      "Эта парольная фраза шифрует каждую резервную копию. Она хранится только в связке ключей этого устройства.",
    passphraseWarning:
      "Если вы потеряете это устройство и парольную фразу, ваши резервные копии не смогут быть восстановлены — никем. Сохраните её в своём менеджере паролей прямо сейчас.",
    passphraseLabel: "Парольная фраза (минимум 8 символов)",
    passphraseRepeat: "Повторите парольную фразу",
    passphraseSaved: "Я сохранил парольную фразу в надёжном месте",
    passphraseMinLength: "Используйте минимум 8 символов.",
    passphraseMismatch: "Парольные фразы не совпадают.",

    // Step 3 - frequency
    backupFrequency: "Частота резервирования",
    destinationSummary: "Назначение:",
    catchupNote:
      "Soma также проверяет при каждом запуске и наверстывает, если резервная копия была пропущена. Сохраняются 12 новейших снимков; более старые удаляются автоматически.",
    enableAndBackup: "Включить и создать резервную копию",

    // Restore dialog
    restoreTitle: "Восстановить из резервной копии",
    restorePickDescription:
      "Выберите .somabk файл и введите парольную фразу, которой он был зашифрован.",
    restoreReviewDescription: "Просмотрите резервную копию перед заменой ваших данных.",
    chooseBackupFile: "Выбрать файл резервной копии…",
    passphrase: "Парольная фраза",
    decryptedSize: "Размер после расшифровки:",
    fileDate: "дата файла:",
    schema: "схема",
    appHas: "в приложении",
    newerVersionError:
      "Эта резервная копия была создана более новой версией Soma. Сначала обновите приложение, затем восстановите.",
    olderSchemaNote:
      "Резервная копия использует более старую схему — она будет автоматически мигрирована после перезапуска.",
    replaceWarningTitle: "Это заменит вашу текущую базу данных и перезапустит приложение.",
    replaceWarningDescription:
      "Резервная копия текущей базы данных с временной меткой сохраняется рядом с ней, поэтому это можно отменить вручную при необходимости.",
    replaceAndRestart: "Заменить данные и перезапустить",

    // Native dialogs
    chooseFolderDialogTitle: "Выберите папку для резервных копий",
    chooseBackupDialogTitle: "Выберите резервную копию Soma",
    somaBackupFilter: "Резервная копия Soma",
  },

  // LabPanelNew page
  labPanelNew: {
    title: "Новая панель анализов",
    description: "Ручной ввод забора лабораторных данных.",
    selectBiomarker: "Выберите биомаркер…",
    addRow: "Добавить строку",
    savePanelSingular: "Сохранить панель ({{count}} результат)",
    savePanelPlural: "Сохранить панель ({{count}} результатов)",
    panelTitle: "Панель",
    resultsTitle: "Результаты",
    fields: {
      labName: "Название лаборатории",
      biomarker: "Биомаркер",
      collectionTime: "Время забора",
      fasting: "Натощак",
      cycleDay: "День цикла",
      notes: "Контекст / метод",
    },
    fasting: { yes: "Натощак", no: "Не натощак", unknown: "Неизвестно" },
    notesPlaceholder: "напр. через 2ч после еды, метод анализа, аккредитация лаб.",
    removeRow: "Удалить строку",
    unitGroups: {
      compatible: "Конвертируемые единицы",
      other: "Другие единицы",
    },
    unitWarning:
      "Неизвестная конвертация в {{unit}} — значение сохранится как введено, без нормализации и флага выхода за норму.",
  },

  // BiomarkerDetail page
  biomarkerDetail: {
    biomarkerNotFound: "Биомаркер не найден",
    emptyTitle: "Пока нет результатов",
    emptyDescription: "Добавьте панель анализов, содержащую этот биомаркер, чтобы увидеть тренд.",
    trendTitle: "Тренд",
    allResultsTitle: "Все результаты",
    allBiomarkers: "Все биомаркеры",
    reference: "Референс",
    optimal: "Оптимум",
    higherBetter: "выше — лучше",
    lowerBetter: "ниже — лучше",
    trendDescription:
      "Затенённые полосы — референсный и оптимальный диапазоны. Включайте лекарства ниже, чтобы наложить периоды приёма и сопоставить их со сдвигами.",
  },

  // График тренда (линия биомаркера + наложения)
  trendChart: {
    unitsNotRecognized:
      "Единицы не распознаны — значения показаны в исходных единицах и не проверены по диапазону.",
    pointsHidden: "Скрыто точек: {{count}} — единица измерения не относится к этой шкале.",
    outOfRange: "вне нормы ({{flag}})",
    severityShort: "тяжесть {{value}}/10",
    symptomLegend:
      "Вертикальные метки — отмеченные симптомы; число — тяжесть по шкале 1–10 (цвет = интенсивность). Наведите на метку для деталей.",
  },

  // Справочная карточка «О показателе»
  biomarkerInfo: {
    title: "О показателе",
    summaryLabel: "Что это",
    highLabel: "О чём говорит повышение",
    lowLabel: "О чём говорит понижение",
    affectsLabel: "На что влияет",
    disclaimer:
      "Данные носят справочный характер и не являются диагнозом. Причины указаны как возможные, а не однозначные. Полагайтесь на рекомендации врача.",
  },

  // ImportWizard page
  importWizard: {
    title: "ИИ-импорт",
    description:
      "PDF или фото → извлечённые значения → вы подтверждаете соответствие → сохранено. Ничего не записывается без вашего просмотра.",
    notMapped: "— не сопоставлено —",
    aiSuggestionBadge: "черновик ИИ",
    saveToFindings: "Сохранить в находки",
    findingsTitle: "Дополнительные находки",
    findingsDescription:
      "Нечисловые результаты из отчёта (например, «отрицательно», титры). Сохраняются с панелью как есть — как биомаркеры не отслеживаются. Отредактируйте или удалите строки перед сохранением.",
    swapDuplicateHint: "Использовать этот результат вместо выбранного",
    aiSuggestionHint:
      "Диалог предзаполнен черновиком определения биомаркера от ИИ на основе напечатанной метки, единицы и референсного диапазона — проверьте каждое поле перед созданием.",
    translatedHint:
      "Сопоставлено по английскому переводу ИИ, а не по печатной метке — проверьте, что биомаркер верный (например, ЛПНП vs общий холестерин).",
    savingPanel: "Сохраняем панель…",
    chooseLabReport: "Выберите отчёт лаборатории",
    anyLangHint: "Любой язык, любая лаборатория, любая страна — включая сканы и фото.",
    chooseAnother: "Выбрать другой",
    extractResults: "Извлечь результаты",
    fileFormats: "PDF, JPG, PNG или WebP",
    chooseFile: "Выбрать файл",
    privacyNotice:
      "Выбранный документ полностью отправляется выбранному вами ИИ-провайдеру для распознавания. Все остальные данные остаются на вашем устройстве.",
    extractingMapping: "Извлечение и сопоставление…",
    extractingMappingDetail:
      "Этап 1: структурированное извлечение. Этап 2: сопоставление со справочником биомаркеров (точное → синонимы → нечёткое → ИИ-разрешение).",
    reviewExtractedResults: "Просмотр извлечённых результатов",
    matchColumn: "Соответствие",
    panelDetailsTitle: "Детали панели",
    duplicateBadge: "дубликат",
    // Проверка на повторный импорт: документ похож на то, что уже сохранено.
    duplicates: {
      title: "Похоже, этот отчёт уже импортирован",
      titlePossible: "Похожая запись уже есть",
      likely: "скорее всего дубликат",
      possible: "возможно, дубликат",
      panelMatch: "Панель от {{date}}, {{lab}} — {{shared}}",
      sharedCount: {
        one: "совпал {{n}} биомаркер",
        few: "совпали {{n}} биомаркера",
        many: "совпало {{n}} биомаркеров",
      },
      unknownLab: "лаборатория не указана",
      rowMatch: "Строка {{row}} ({{name}}, {{date}}) уже записана как «{{existing}}»",
      rowBadge: "уже есть",
      explain:
        "Два забора в один день — это нормально. Но один и тот же PDF, импортированный дважды, удваивает каждую точку в трендах. Загляните в существующую запись, прежде чем сохранять.",
      openExisting: "Открыть существующую запись",
      importAnyway: "Всё равно импортировать",
      acknowledged: "Вы решили импортировать, хотя это может быть дубликат.",
      savedSibling:
        "В записях есть ещё одна панель за этот день: {{date}}, {{lab}} — {{shared}}. Если это один и тот же отчёт, импортированный дважды, удалите одну из них.",
    },
    duplicateGroupHint:
      "Две строки сопоставлены с одним биомаркером. По умолчанию оставлено лучшее совпадение — при необходимости выберите другое.",
    duplicatesUnresolved:
      "У {{count}} биомаркера(ов) выбрано несколько строк-дубликатов — оставьте по одной.",
    aiDisabledTitle: "ИИ-анализ отключён",
    aiDisabledDescription:
      "Чтобы импортировать отчёты лабораторий из PDF или фотографий, включите ИИ-анализ и вставьте свой API-ключ в настройках. Ваши документы отправляются только выбранному провайдеру и только когда вы запускаете импорт.",
    openSettings: "Открыть настройки",
    selectType: {
      title: "Что вы импортируете?",
      description: "Выберите тип документа, чтобы ИИ знал, что искать.",
      continue: "Продолжить",
    },
    docTypes: {
      lab: "Отчёт лаборатории",
      labDescription: "Кровь, моча и другие количественные результаты.",
      vaccine: "Сертификат о прививках",
      vaccineDescription: "Дозы вакцин, даты, производители и серии.",
      discharge: "Выписной эпикриз",
      dischargeDescription: "Визит, диагнозы, лекарства и аллергии.",
      imaging: "Заключение по снимкам",
      imagingDescription: "Лучевые исследования — рентген, КТ, МРТ, УЗИ — и их описание.",
      prescription: "Рецепт",
      prescriptionDescription: "Назначенные препараты и добавки с дозой и режимом приёма.",
      allergy: "Запись об аллергиях",
      allergyDescription: "Аллергии и непереносимости с реакцией и тяжестью.",
    },
    choosePrompt: "Выберите: {{type}}",
    extractingDoc: "Читаем документ…",
    verifyBadge: "проверить",
    reviewBanner: {
      title: "Требуется полная ручная проверка",
      description:
        "Сверьте каждую запись с оригиналом перед сохранением. Ничего не сохраняется автоматически.",
    },
    vaccineReview: {
      choose: "Выберите сертификат о прививках",
      extracting: "Читаем сертификат…",
      title: "Проверьте извлечённые прививки",
      description:
        "Каждая доза сопоставлена с вакциной из вашего календаря — подтвердите сопоставление, отметьте нужные дозы и при необходимости исправьте поля по оригиналу.",
      saving: "Сохраняем прививки…",
      selectAll: "Выбрать все",
      noMatch: "Нет совпадения",
      columns: {
        vaccine: "Как в документе",
        match: "Вакцина из календаря",
        dose: "Доза №",
        manufacturer: "Производитель",
        batch: "Серия",
        expires: "Действует до",
      },
      empty: "В документе не найдено ни одной дозы вакцины.",
      save: "Сохранить прививок: {{count}}",
    },
    dischargeReview: {
      choose: "Выберите выписной эпикриз",
      extracting: "Читаем выписной эпикриз…",
      title: "Проверьте выписной эпикриз",
      description: "Подтвердите данные визита и выберите, какие записи сохранить.",
      saving: "Сохраняем записи…",
      visitTitle: "Данные визита",
      typeColumn: "Тип",
      detailColumn: "Детали",
      diagnosisBadge: "диагноз",
      medicationBadge: "лекарство",
      allergyBadge: "аллергия",
      empty: "В документе не найдено диагнозов, лекарств или аллергий.",
      save: "Сохранить записей: {{count}}",
    },
    imagingReview: {
      title: "Проверьте исследования",
      description: "Отметьте исследования для сохранения и исправьте поля по заключению.",
      empty: "В документе не найдено лучевых исследований.",
      columns: {
        date: "Дата",
        modality: "Модальность",
        bodyArea: "Область",
        findings: "Заключение",
        facility: "Учреждение",
      },
      save: "Сохранить исследований: {{count}}",
    },
    prescriptionReview: {
      title: "Проверьте назначенные лекарства",
      description: "Отметьте лекарства для сохранения и исправьте дозу, тип и режим приёма.",
      empty: "В документе не найдено лекарств.",
      columns: {
        name: "Лекарство",
        type: "Тип",
        dose: "Доза",
        schedule: "Режим приёма",
        purpose: "Назначение",
      },
      duplicate: "уже отслеживается",
      duplicateHint: "Лекарство с таким названием уже есть в вашем списке.",
      save: "Сохранить лекарств: {{count}}",
    },
    allergyReview: {
      title: "Проверьте аллергии",
      description: "Отметьте аллергии для сохранения и подтвердите категорию и тяжесть.",
      empty: "В документе не найдено аллергий.",
      columns: {
        allergen: "Аллерген",
        category: "Категория",
        severity: "Тяжесть",
        reaction: "Реакция",
      },
      duplicate: "уже записано",
      duplicateHint: "Аллергия на этот аллерген уже записана.",
      save: "Сохранить аллергий: {{count}}",
    },
  },

  // Связь с исходным документом, из которого был импорт
  sourceFile: {
    title: "Исходный документ",
    attached: "Файл-источник",
    open: "Открыть оригинал",
    openInApp: "Посмотреть оригинал",
    openExternally: "Открыть в системе",
    revealInFinder: "Показать в папке",
    page: "Стр. {{n}}",
    fromPage: "со стр. {{n}}",
    none: "Нет файла-источника",
    manualEntry: "Введено вручную — исходного документа нет.",
    missing: "Не удалось найти исходный файл. Возможно, он перемещён или удалён.",
    previewUnsupported: "Для этого типа файла встроенный просмотр недоступен.",
    pdf: "PDF-документ",
    image: "Изображение",
    close: "Закрыть",
    loading: "Загрузка документа…",
  },

  // Постоянное состояние «требует проверки» для неуверенных ИИ-сопоставлений
  needsReview: {
    badge: "на проверку: {{count}}",
    one: "1 результат требует проверки",
    many: "Результатов на проверку: {{count}}",
    panelTitle: "Часть результатов требует вашей проверки",
    panelDescription:
      "Эти сопоставления были неуверенными — откройте оригинал, сверьте каждое значение и подтвердите.",
    rowHint: "Неуверенно — сверьте с источником",
    confirm: "Подтвердить",
    confirmRow: "Подтвердить сопоставление",
    confirmAll: "Подтвердить все",
    reviewed: "Проверено",
    allReviewed: "Все результаты проверены",
    verifyAction: "Проверить и подтвердить",
    globalTitle: "Импорты, ожидающие проверки",
    globalOne: "1 импорт с непроверенными результатами",
    globalMany: "Импортов с непроверенными результатами: {{count}}",
    globalCta: "Проверить",
    confirmedToast: "Отмечено как проверенное",
  },

  // Экран сверки «документ ⟷ извлечённые значения»
  verify: {
    title: "Сверка импорта",
    description: "Сверьте каждое извлечённое значение с оригиналом документа.",
    sourcePane: "Оригинал документа",
    resultsPane: "Извлечённые результаты",
    onlyUncertain: "Только строки на проверку",
    showAll: "Показать все строки",
    confirm: "Подтвердить",
    confirmAll: "Подтвердить оставшиеся",
    edit: "Изменить сопоставление",
    done: "Готово",
    backToPanel: "К панели",
    empty: "Проверять больше нечего.",
    jumpToPage: "Перейти на стр. {{n}}",
  },

  // Типизированные ошибки ИИ-импорта (вместо одного общего баннера)
  importErrors: {
    authTitle: "Ключ API отклонён",
    authBody: "Провайдер ИИ отклонил ключ. Проверьте его в настройках.",
    authAction: "Открыть настройки",
    rateLimited: "Провайдер ограничивает частоту запросов.",
    overloaded: "Провайдер ИИ перегружен.",
    retrying: "Временная ошибка — повтор ({{attempt}}/{{max}})…",
    network: "Не удалось связаться с провайдером ИИ. Проверьте соединение.",
    retry: "Повторить",
    badDocumentTitle: "Результаты не найдены",
    badDocumentBody:
      "Не удалось найти значения в этом файле. Тот ли это тип документа и читается ли скан?",
    switchType: "Сменить тип документа",
    truncatedTitle: "Документ, возможно, слишком большой",
    truncatedBody:
      "Ответ был обрезан, часть результатов может отсутствовать. Проверьте внимательно или разделите файл.",
    parseFailed: "ИИ вернул нечитаемый ответ. Попробуйте ещё раз.",
    tooLarge:
      "Документ слишком большой для ИИ-провайдера. Разделите PDF или импортируйте меньше страниц за раз.",
    fileTooLarge:
      "Этот файл больше {{limit}} МБ — слишком велик для отправки ИИ-провайдеру. Разделите PDF или используйте скан меньшего размера.",
    badRequest:
      "ИИ-провайдер отклонил запрос. Выбранная модель может не поддерживать PDF или изображения — выберите модель с поддержкой изображений в настройках.",
    droppedRows:
      "Пропущено нечисловых строк: {{count}} (например, положительно/отрицательно, титры).",
    showDropped: "Показать пропущенные строки",
    dateNotRecognized: "Не удалось распознать дату забора — поставлена сегодняшняя. Укажите её.",
    truncated:
      "Ответ ИИ упёрся в лимит длины и оборвался — строки после обрыва не попали сюда. Сверьтесь с документом и добавьте недостающее вручную.",
    genericTitle: "Импорт не удался",
  },

  // Dashboard page
  dashboard: {
    noRecordsTitle: "Пока нет записей",
    recentActivityTitle: "Последняя активность",
    fullTimeline: "Вся шкала",
    addLabResults: "Добавить анализы",
    importLab: "Импортировать анализ",
    empty: {
      title: "Главный экран скрыт",
      description:
        "Вы отключили все карточки дашборда. Включите хотя бы одну, чтобы заполнить экран.",
      cta: "Выбрать карточки",
    },
    stats: {
      labPanels: "Панели анализов",
      outOfRangeLatest: "Вне диапазона (последняя панель)",
      activeMedications: "Активные лекарства",
      lastLabDraw: "Последний забор крови",
    },
    recentActivity: {
      description:
        "Добавьте свою первую панель анализов, лекарство или визит к врачу, чтобы начать.",
    },
    verdict: {
      calm: "Всё в норме",
      attentionOne: "1 показатель требует внимания",
      attentionMany: "{{count}} показателей требуют внимания",
    },
    safety: {
      label: "Критическая аллергия",
    },
    attention: {
      title: "Требует внимания",
      biomarkersOne: "1 показатель ухудшился в последней панели",
      biomarkersMany: "{{count}} показателей ухудшились в последней панели",
      diagnosesOne: "1 активный диагноз: {{names}}",
      diagnosesMany: "Активных диагнозов: {{count}} — {{names}}",
      medsEndingOne: "1 лекарство скоро заканчивается: {{names}}",
      medsEndingMany: "Скоро заканчиваются лекарства ({{count}}): {{names}}",
      vaccinesOne: "1 прививка требует действия",
      vaccinesMany: "Прививок требует действия: {{count}}",
      reviewOne: "1 импорт с непроверенными результатами",
      reviewMany: "Импортов с непроверенными результатами: {{count}}",
    },
  },

  // VisitDetail page
  visitDetail: {
    visitNotFound: "Визит не найден",
    deleteVisit: "Удалить визит",
    addPrescription: "Добавить назначение",
    diagnosesTitle: "Диагнозы",
    prescriptionsTitle: "Назначения",
    fields: {
      prescription: "Назначение",
      drugName: "Препарат / добавка",
      durationDays: "Длительность (дни)",
      days: "дн.",
    },
    medicationsTitle: "Лекарства",
    noMedications: "К этому визиту не привязаны лекарства.",
  },

  // Страница диагноза
  diagnosisDetail: {
    notFound: "Диагноз не найден",
    detailsTitle: "Детали",
  },

  // Страница лекарства
  medicationDetail: {
    notFound: "Лекарство не найдено",
    detailsTitle: "Детали",
    endDate: "Дата окончания",
  },

  // Блоки «Связано» между сущностями (визит ↔ диагноз ↔ лекарство)
  related: {
    title: "Связано",
    fromVisit: "Из визита",
    treatedBy: "Лечится",
    treats: "Лечит",
    prescribedAt: "Назначено на визите",
    visit: "Визит",
  },

  // Полнотекстовый поиск + командная палитра
  search: {
    title: "Поиск",
    open: "Поиск",
    placeholder: "Записи, разделы, действия…",
    timelineFilter: "Фильтр событий…",
    esc: "Esc",
    hint: "Начните печатать — или выберите раздел ниже",
    noResults: "Ничего не найдено по запросу «{{query}}»",
    noResultsHint: "Попробуйте покороче или кусок названия — опечатки поиск переживёт.",
    footer: "↑↓ выбрать · Tab группа · Enter открыть · Esc закрыть",
    resultCount: {
      one: "{{count}} совпадение",
      few: "{{count}} совпадения",
      many: "{{count}} совпадений",
    },
    groups: {
      commands: "Перейти",
      recent: "Недавнее",
      nav: "Раздел",
      action: "Действие",
    },
    commands: {
      newLabPanel: "Новая лаб-панель",
      importLabs: "Импорт анализов",
      compareLabs: "Сравнить панели",
      newImaging: "Добавить снимок",
      emergencyCard: "Карта экстренной помощи",
      settings: "Настройки",
    },
    types: {
      biomarker: "Биомаркер",
      lab_panel: "Анализы",
      lab_result: "Результат",
      lab_finding: "Находка",
      visit: "Визит",
      diagnosis: "Диагноз",
      medication: "Лекарство",
      prescription: "Назначение",
      allergy: "Аллергия",
      vaccine: "Вакцина",
      symptom: "Симптом",
      imaging: "Снимок",
      health_note: "Заметка",
      weight_log: "Вес",
      bp_log: "Давление",
      lifestyle_log: "Образ жизни",
      retest: "Пересдача",
      attachment: "Файл",
      chat_thread: "Чат",
    },
  },

  // Emergency card
  emergency: {
    title: "Карта экстренной помощи",
    description: "Печатная сводка важной медицинской информации для врачей скорой помощи.",
    exportHtml: "Экспорт в HTML",
    exportPdf: "Экспорт в PDF",
    openCard: "Карта экстренной помощи",
    incompleteBanner:
      "Не указана группа крови или контакт для экстренной связи — карта может быть неполной.",
    updateProfile: "Обновить профиль →",
    exportError: "Не удалось экспортировать карту. Попробуйте ещё раз.",
    sections: {
      identity: "Личные данные",
      criticalStatus: "Критический статус",
      contact: "Экстренный контакт",
      insurance: "Страховка и помощь",
      notes: "Важные заметки",
      allergies: "Аллергии",
      medications: "Активные лекарства",
      diagnoses: "Активные диагнозы",
      vaccines: "Недавние прививки",
    },
    criticalStatus: {
      pregnancy: "Беременность",
      codeStatus: "Реанимация",
      organDonor: "Донор органов",
      yes: "Да",
      no: "Нет",
      pregnancyValues: {
        not_pregnant: "Не беременна",
        pregnant: "Беременна",
        postpartum: "Послеродовой период",
        unknown: "Неизвестно",
      },
      codeStatusValues: {
        full_code: "Полная реанимация",
        dnr: "DNR (не реанимировать)",
        dni: "DNI (не интубировать)",
      },
    },
    identity: {
      name: "Имя",
      dob: "Дата рождения",
      age: "Возраст",
      ageValue: "{{years}} лет",
      sex: "Пол",
      bloodType: "Группа крови",
      citizenship: "Гражданство",
      languages: "Языки",
    },
    insurance: {
      insurer: "Страховая",
      policyNumber: "Номер полиса",
      phone: "Помощь 24/7",
    },
    emptyInsurance: "Страховка не указана — добавьте в настройках.",
    contact: {
      name: "Имя",
      phone: "Телефон",
      relation: "Кем приходится",
      empty: "Экстренный контакт не указан — добавьте его в настройках.",
    },
    allergies: {
      allergen: "Аллерген",
      severity: "Тяжесть",
      reaction: "Реакция",
      resolved: "Разрешилась",
      none: "Известных аллергий не зафиксировано.",
    },
    medications: {
      name: "Лекарство",
      dose: "Доза",
      schedule: "Приём",
      since: "С",
      none: "Активных лекарств нет.",
      asNeededTitle: "По потребности (PRN)",
    },
    diagnoses: {
      name: "Диагноз",
      icd: "МКБ",
      date: "Дата",
      none: "Активных диагнозов нет.",
    },
    vaccines: {
      name: "Прививка",
      date: "Дата",
      dose: "Доза",
      doseValue: "№{{n}}",
      expires: "Действует до",
      expired: "Истекла",
      none: "Прививок не зафиксировано.",
    },
    footer:
      "Сформировано в Soma {{date}}. Данные актуальны на момент экспорта. Всегда уточняйте у лечащего врача.",
    settings: {
      title: "Экстренная информация",
      description: "Отображается в карте экстренной помощи для врачей.",
      name: "Имя контакта",
      phone: "Телефон",
      relation: "Кем приходится",
      citizenship: "Гражданство",
      citizenshipPlaceholder: "напр. Россия",
      languages: "Языки",
      languagesPlaceholder: "напр. русский, английский",
      insurer: "Страховая",
      policyNumber: "Номер полиса",
      assistancePhone: "Телефон помощи",
      notes: "Критичные заметки",
      notesHint: "Важно для врачей скорой: кардиостимулятор, импланты, отказ от переливания…",
      pregnancy: "Беременность",
      codeStatus: "Реанимация",
      organDonor: "Донор органов",
      notSet: "Не указано",
      save: "Сохранить",
      saved: "Сохранено",
    },
  },

  // Journal page (weight / blood pressure / symptoms)
  journal: {
    title: "Дневник",
    description: "Отслеживайте вес, давление и симптомы во времени.",
    tabs: {
      overview: "Обзор",
      weight: "Вес",
      bp: "Давление",
      symptoms: "Симптомы",
    },
    overview: {
      activity: "Журнал событий",
      noData: "Нет записей за период",
      emptyTitle: "Пока ничего не записано",
      emptyDescription: "Записывайте вес, давление или симптомы — здесь они появятся вместе.",
      entries: "записей: {{n}}",
      pulse: "пульс {{n}}",
      ranges: {
        "3m": "3 мес",
        "6m": "6 мес",
        "1y": "1 год",
        all: "Всё",
      },
    },
  },

  weight: {
    chartTitle: "Динамика веса",
    targetLabel: "Цель",
    logWeight: "Записать вес",
    emptyTitle: "Записей о весе пока нет",
    emptyDescription: "Запишите вес, чтобы видеть его динамику во времени.",
    addFirst: "Записать первый вес",
    table: {
      date: "Дата",
      weight: "Вес",
      notes: "Заметки",
    },
    dialog: {
      titleAdd: "Записать вес",
      titleEdit: "Изменить запись веса",
    },
    fields: {
      weight: "Вес",
      notesOptional: "Заметки (необязательно)",
    },
  },

  weightGoal: {
    set: "Поставить цель",
    titleAdd: "Цель по весу",
    titleEdit: "Изменить цель по весу",
    description:
      "Строит линию-план от стартового веса к цели, чтобы видеть, идёте ли вы с опережением или отстаёте.",
    startWeight: "Стартовый вес",
    startDate: "Дата старта",
    targetWeight: "Целевой вес",
    targetDate: "Дата цели",
    save: "Поставить цель",
    remove: "Удалить цель",
    removedToast: "Цель по весу удалена",
    today: "Сегодня",
    planLabel: "План",
    ahead: "опережение {{v}}",
    behind: "отставание {{v}}",
    onTrack: "по плану",
    expired: "срок цели истёк",
    upcoming: "ещё не началась",
  },

  bp: {
    chartTitle: "Динамика давления",
    systolic: "Систолическое",
    diastolic: "Диастолическое",
    logReading: "Записать измерение",
    emptyTitle: "Измерений давления пока нет",
    emptyDescription: "Запишите измерение, чтобы отслеживать давление во времени.",
    addFirst: "Записать первое измерение",
    crisisBanner:
      "Измерение за последние 7 дней находится в диапазоне гипертонического криза (систолическое > 180 или диастолическое > 120). При таких симптомах, как боль в груди, одышка или нарушение зрения, немедленно обратитесь за неотложной медицинской помощью.",
    status: {
      stage2: "2 стадия",
      crisis: "Криз",
    },
    crisisHint: "Это значение в диапазоне гипертонического криза.",
    table: {
      date: "Дата",
      time: "Время",
      reading: "Измерение",
      pulse: "Пульс",
      status: "Статус",
      notes: "Заметки",
    },
    dialog: {
      titleAdd: "Записать давление",
      titleEdit: "Изменить измерение",
    },
    fields: {
      timeOptional: "Время (необязательно)",
      systolic: "Систолическое",
      diastolic: "Диастолическое",
      heartRate: "Пульс",
      position: "Положение",
      arm: "Рука",
      notesOptional: "Заметки (необязательно)",
    },
    position: {
      sitting: "Сидя",
      standing: "Стоя",
      supine: "Лёжа",
    },
    arm: {
      left: "Левая",
      right: "Правая",
    },
  },

  symptoms: {
    thresholdLabel: "Показаны с тяжестью ≥ {{n}}",
    showAll: "Показать все",
    logSymptom: "Записать симптом",
    emptyTitle: "Симптомы пока не записаны",
    emptyDescription: "Записывайте симптомы с тяжестью, чтобы замечать закономерности.",
    addFirst: "Записать первый симптом",
    previouslyLogged: "Ранее записанные",
    table: {
      date: "Дата",
      time: "Время",
      symptom: "Симптом",
      severity: "Тяжесть",
      notes: "Заметки",
    },
    dialog: {
      titleAdd: "Записать симптом",
      titleEdit: "Изменить симптом",
    },
    fields: {
      symptomName: "Симптом",
      timeOptional: "Время (необязательно)",
      severity: "Тяжесть",
      notesOptional: "Заметки (необязательно)",
    },
    namePlaceholder: "напр. Головная боль, Усталость",
  },

  symptomSeverity: {
    "1": "1 — Минимальная",
    "2": "2 — Очень лёгкая",
    "3": "3 — Лёгкая",
    "4": "4 — Лёгкая-умеренная",
    "5": "5 — Умеренная",
    "6": "6 — Умеренно-сильная",
    "7": "7 — Сильная",
    "8": "8 — Очень сильная",
    "9": "9 — Интенсивная",
    "10": "10 — Невыносимая",
  },

  // Imaging records
  imaging: {
    title: "Снимки",
    description: "Рентген, КТ, МРТ, УЗИ и другие исследования.",
    newRecord: "Новая запись",
    emptyTitle: "Записей о снимках пока нет",
    emptyDescription: "Добавьте исследование, чтобы хранить историю снимков в одном месте.",
    addFirst: "Добавить первую запись",
    back: "Снимки",
    table: {
      date: "Дата",
      modality: "Тип",
      bodyArea: "Область тела",
      facility: "Учреждение",
      visit: "Визит",
    },
    newTitle: "Новая запись о снимке",
    editTitle: "Изменить запись о снимке",
    newDescription: "Запишите исследование и его заключение.",
    studyDetailsTitle: "Сведения об исследовании",
    findingsTitle: "Заключение",
    delete: "Удалить запись",
    fields: {
      date: "Дата",
      modality: "Тип",
      bodyArea: "Область тела",
      facility: "Учреждение",
      city: "Город",
      country: "Страна",
      visit: "Визит",
      findings: "Заключение",
      radiologist: "Рентгенолог",
    },
    bodyAreaPlaceholder: "напр. Поясничный отдел, Грудная клетка",
    noVisit: "Без привязки к визиту",
    visitLabel: "{{date}} — {{doctor}}",
    visitLabelNoDoctor: "{{date}} — визит",
  },

  imagingModality: {
    xray: "Рентген",
    ct: "КТ",
    mri: "МРТ",
    ultrasound: "УЗИ",
    pet: "ПЭТ",
    other: "Другое",
  },

  biomarkerSymptoms: {
    toggle: "Симптомы",
  },

  breadcrumb: {
    back: "Назад",
    labPanelNew: "Новая панель",
    importWizard: "ИИ-импорт",
    verify: "Проверка",
    imagingNew: "Новый снимок",
    imagingEdit: "Изменить снимок",
  },

  // Контекст образа жизни (сон / тренировки / стресс) — обогащение для ИИ v0.3
  lifestyle: {
    title: "Образ жизни",
    description:
      "Ежедневный сон, тренировки и стресс — контекст, который делает ваши тренды и ИИ-анализ точнее.",
    logToday: "Записать сегодня",
    logEntry: "Записать день",
    emptyTitle: "Записей об образе жизни пока нет",
    emptyDescription:
      "Записывайте, как вы спали, тренировались и чувствовали себя. Это обогащает ваши тренды и Soma AI.",
    addFirst: "Записать первый день",
    aiNote: "Записанный контекст образа жизни учитывается в Soma AI и при интерпретации трендов.",
    windowNote: "Средние за последние {{days}} дн., по {{count}} записанным дням.",
    noWindowData: "Нет записей за последние {{days}} дн.",
    cards: {
      sleep: "Сон",
      training: "Тренировки",
      stress: "Стресс и энергия",
    },
    metrics: {
      avgSleep: "Средний сон",
      avgSleepQuality: "Качество сна",
      trainingLoad: "Тренировки",
      trainingDays: "{{count}} дн.",
      avgStress: "Средний стресс",
      avgEnergy: "Средняя энергия",
      restingHr: "Пульс покоя",
      steps: "Средние шаги",
      hours: "{{n}} ч",
      minutes: "{{n}} мин",
      perWeek: "{{n}} / нед.",
      scale5: "{{n}}/5",
      bpm: "{{n}} уд/мин",
      noData: "—",
    },
    fields: {
      date: "Дата",
      sleepHours: "Сон (часы)",
      sleepQuality: "Качество сна",
      trainingMinutes: "Тренировки (минуты)",
      trainingIntensity: "Интенсивность",
      steps: "Шаги",
      restingHeartRate: "Пульс покоя",
      stressLevel: "Уровень стресса",
      energyLevel: "Уровень энергии",
      notesOptional: "Заметки (необязательно)",
      notSet: "—",
    },
    intensity: {
      light: "Лёгкая",
      moderate: "Умеренная",
      intense: "Интенсивная",
    },
    scale: {
      "1": "1 — очень низкий",
      "2": "2 — низкий",
      "3": "3 — умеренный",
      "4": "4 — высокий",
      "5": "5 — очень высокий",
    },
    table: {
      date: "Дата",
      sleep: "Сон",
      training: "Тренировки",
      stress: "Стресс",
      energy: "Энергия",
      notes: "Заметки",
    },
    dialog: {
      titleAdd: "Записать день образа жизни",
      titleEdit: "Изменить день образа жизни",
    },
    integrations: {
      title: "Подключённые источники",
      comingSoon: "Скоро",
      body: "Синхронизация с Apple Health, Google Fit и Health Connect в планах. Пока вводите сон, тренировки и стресс вручную — модель данных уже подготовлена под будущую автоматическую синхронизацию.",
      docNote: "Описание интеграции см. в docs/health-data-integrations.md.",
    },
  },

  // Лента уведомлений (только внутри приложения — никогда не системные уведомления)
  notifications: {
    title: "Уведомления",
    description:
      "Напоминания о приёме лекарств и запланированные повторные анализы — только внутри приложения, ничего не покидает ваше устройство.",
    emptyTitle: "Всё под контролем",
    emptyDescription:
      "Напоминания о лекарствах и повторные анализы, которые пора сдать, появятся здесь.",
    feedTitle: "Лента",
    prefs: {
      title: "О чём вас уведомлять",
      description:
        "Отключите категории, которые вам не нужны. Действует на ленту и счётчик у колокольчика.",
      medication: {
        label: "Напоминания о лекарствах",
        desc: "Ежедневное напоминание о постоянных препаратах, которые вы ещё не отметили.",
      },
      retest: {
        label: "Напоминания о повторных анализах",
        desc: "Сигнал, когда пора пересдать запланированный показатель или панель.",
      },
      retestUpcoming: {
        label: "Показывать предстоящие",
        desc: "Показывать и анализы, которые скоро пора сдать — отключите, чтобы видеть только наступившие и просроченные.",
      },
    },
    dismiss: "Скрыть",
    dismissed: "Скрыто",
    showDismissed: "Показать скрытые ({{count}})",
    hideDismissed: "Скрыть скрытые",
    restore: "Вернуть",
    medication: {
      title: "Примите {{name}}",
      due: "Запланировано на сегодня — ещё не отмечено",
      atTimes: "В {{times}}",
      open: "Лекарства",
    },
    retest: {
      title: "Повторный анализ: {{label}}",
      dueToday: "Пора сдать сегодня",
      due: "Пора сдать {{date}}",
      overdue: "Просрочено на {{days}} дн.",
      upcoming: "Сдать через {{days}} дн.",
      noAnchor: "Укажите дату последнего анализа, чтобы запустить цикл",
      open: "Открыть",
    },
    schedules: {
      title: "Графики повторного тестирования",
      description:
        "Задайте цикл, чтобы напоминать о повторной проверке показателя раз в несколько месяцев.",
      add: "Добавить график",
      emptyTitle: "Графиков повторных анализов нет",
      emptyDescription:
        "Добавьте показатель или панель, которые хотите перепроверять с регулярным циклом.",
      every: "Каждые {{n}} мес.",
      nextDue: "Следующий срок",
      lastTested: "Последний анализ",
      status: "Статус",
      active: "Активен",
      paused: "На паузе",
      pause: "Пауза",
      resume: "Возобновить",
      table: {
        label: "Что",
        every: "Цикл",
        lastTested: "Последний анализ",
        nextDue: "Следующий срок",
        status: "Статус",
      },
      dialog: {
        titleAdd: "Добавить график повторного анализа",
        titleEdit: "Изменить график повторного анализа",
      },
      fields: {
        label: "Метка",
        labelPlaceholder: "напр. Липидный профиль, Витамин D, ТТГ",
        biomarker: "Связанный биомаркер (необязательно)",
        biomarkerNone: "Нет",
        intervalMonths: "Повторять каждые (месяцы)",
        lastTestedDate: "Последний анализ (необязательно)",
        notesOptional: "Заметки (необязательно)",
      },
    },
  },

  // Отчёт для врача (генератор PDF)
  doctorReport: {
    title: "Отчёт для врача",
    description: "Сформируйте аккуратную PDF-сводку вашей карты, чтобы поделиться с врачом.",
    intro:
      "Печатная сводка ваших текущих проблем, лекарств, аллергий и недавних результатов. Создаётся локально на вашем устройстве — ничего не загружается в сеть.",
    generate: "Сформировать PDF",
    generating: "Формируем…",
    rangeLabel: "Период истории",
    range: {
      last6m: "Последние 6 месяцев",
      last12m: "Последние 12 месяцев",
      all: "За всё время",
      custom: "Произвольный",
    },
    from: "С",
    to: "По",
    sectionsLabel: "Включить разделы",
    sections: {
      diagnoses: "Диагнозы",
      medications: "Лекарства",
      allergies: "Аллергии",
      labs: "Анализы",
      visits: "Визиты",
      imaging: "Снимки",
      vaccines: "Прививки",
      lifestyle: "Образ жизни",
    },
    saved: "Отчёт сохранён",
    cancelled: "Экспорт отменён",
    pdf: {
      title: "Сводка о здоровье",
      preparedFor: "Подготовлено для {{name}}",
      generatedOn: "Сформировано {{date}}",
      rangeAll: "Полная история",
      rangeFromTo: "{{from}} – {{to}}",
      rangeUpTo: "До {{to}}",
      rangeFrom: "С {{from}}",
      none: "Не зафиксировано",
      identity: {
        heading: "Пациент",
        dob: "Дата рождения",
        age: "{{years}} л.",
        sex: "Пол",
        bloodType: "Группа крови",
      },
      diagnoses: {
        heading: "Диагнозы",
        active: "Активные",
        inactive: "Прошлые / разрешённые",
        since: "с {{date}}",
      },
      medications: {
        heading: "Лекарства",
        current: "Текущие",
        asNeeded: "По необходимости",
        past: "Прошлые",
        since: "с {{date}}",
        until: "до {{date}}",
      },
      allergies: {
        heading: "Аллергии",
        resolved: "разрешена",
      },
      abnormal: {
        heading: "Показатели вне нормы (последние)",
      },
      labs: {
        heading: "Анализы",
        panel: "{{name}} · {{date}}",
        panelNoName: "Панель анализов · {{date}}",
        marker: "Показатель",
        value: "Значение",
        reference: "Референс",
        outOfRange: "вне нормы",
      },
      visits: {
        heading: "Визиты",
      },
      imaging: {
        heading: "Снимки",
      },
      vaccines: {
        heading: "Прививки",
        dose: "доза {{n}}",
      },
      lifestyle: {
        heading: "Контекст образа жизни",
        summary: "Средние за {{count}} записанных дней",
        sleep: "Сон {{n}} ч",
        sleepQuality: "Качество сна {{n}}/5",
        training: "Тренировки всего {{n}} мин",
        stress: "Стресс {{n}}/5",
        energy: "Энергия {{n}}/5",
        restingHr: "Пульс покоя {{n}} уд/мин",
      },
      flags: {
        low: "низкое",
        high: "высокое",
        critical: "критично",
      },
      disclaimer:
        "Сформировано в Soma из самостоятельно введённых данных. Это личная сводка, а не официальный медицинский документ и не медицинская консультация. Сверяйте с исходными документами.",
    },
  },

  // Шифрование базы данных в покое (опционально)
  dbEncryption: {
    title: "Шифрование базы данных",
    description: "Шифруйте локальную базу данных в покое. Опционально, по умолчанию выключено.",
    statusOff: "Выкл.",
    statusOn: "Вкл.",
    on: "Шифрование включено",
    off: "Ваша база данных не зашифрована в покое.",
    howItWorks:
      "Когда включено, Soma хранит базу данных зашифрованной на диске (Argon2id + AES-256-GCM), пока приложение закрыто, и расшифровывает её при запуске. Данные находятся в открытом виде только во время работы приложения.",
    modeLabel: "Режим разблокировки",
    modeKeychain: "Автоматически (хранилище ключей ОС)",
    modeKeychainDesc:
      "Ключ хранится в хранилище ключей ОС; Soma разблокируется автоматически при запуске. Защищает от офлайн-кражи диска. Минимум трений.",
    modePassphrase: "Парольная фраза при запуске",
    modePassphraseDesc:
      "Вы вводите парольную фразу при каждом запуске; ключ никогда не пишется на диск. Самый надёжный режим — устойчив к компрометации хранилища ключей.",
    passphrase: "Парольная фраза",
    passphraseConfirm: "Подтвердите парольную фразу",
    passphraseHint: "Минимум 8 символов. Если вы её забудете, восстановить будет невозможно.",
    enable: "Включить шифрование",
    disable: "Выключить шифрование",
    enabling: "Включаем…",
    disabling: "Выключаем…",
    changeMode: "Сменить режим",
    warning:
      "Если вы забудете парольную фразу, ваши зашифрованные данные невозможно будет восстановить — никем, включая разработчиков Soma.",
    mismatch: "Парольные фразы не совпадают.",
    tooShort: "Парольная фраза должна быть не короче 8 символов.",
    enabledToast: "Шифрование базы данных включено",
    disabledToast: "Шифрование базы данных выключено",
    modeChangedToast: "Режим разблокировки обновлён",
    error: "Не удалось изменить настройки шифрования",
    requiresKeychainNote:
      "Шифрование хранит свой ключ в хранилище ключей ОС, поэтому требуется рабочее хранилище ключей.",
    keychainUpdateWarning:
      "Ключ привязан к подписи Soma. Soma подписывается ad-hoc, без платного сертификата Apple, поэтому после обновления система может увидеть другое приложение и отказаться отдать ключ. Сохраните ключ восстановления — это займёт несколько секунд и решает, вернётесь ли вы в приложение сразу или останетесь без доступа к своим записям.",
    showRecoveryKey: "Показать ключ восстановления",
    recoveryKeyTitle: "Сохраните ключ восстановления",
    recoveryKeyIntro:
      "Это ключ, которым расшифровывается ваша база. Запишите его или положите в менеджер паролей. Любой, у кого он есть, прочитает ваши медицинские данные, а без него обновление, сломавшее доступ к связке ключей, оставит записи недоступными.",
    recoveryKeySaved: "Я сохранил этот ключ в надёжном месте",
    recoveryKeyDone: "Готово",
  },

  // Стартовый экран восстановления: зашифрованные данные, которые Soma не смогла
  // открыть. Показывается ВМЕСТО онбординга — хранилище на диске означает, что
  // у пользователя есть данные, а не первый запуск.
  vaultBlocked: {
    title: "Данные зашифрованы, и Soma не может их открыть",
    safe: "Всё по-прежнему лежит у вас на диске. Soma ничего не удалила, не изменила и не сбросила — и не станет заводить новую базу, пока рядом лежат ваши зашифрованные данные.",
    likelyCause: "Что произошло",
    cause: {
      probeFailed:
        "Soma не смогла проверить, зашифрована ли база, и остановилась — чтобы не завести пустую базу поверх ваших записей.",
      vaultUnreadable:
        "Зашифрованный файл на месте, но Soma не может прочитать его заголовок. Возможно, он записан более новой версией Soma или повреждён при копировании.",
      keyMissing:
        "База зашифрована ключом из системной связки ключей, и этого ключа там больше нет.",
      keyUnavailable:
        "База зашифрована ключом из системной связки ключей. Ключ на месте — система просто отказалась отдать его этой копии Soma.",
      plaintextSuspicious:
        "Рядом с хранилищем лежит незашифрованная база, но она слишком мала, чтобы быть вашей — скорее всего, пустая и созданная по ошибке. Soma не станет открывать её вместо ваших настоящих данных.",
      unlockFailed:
        "Ключ нашёлся, но расшифровать не удалось. Возможно, ключ от другого хранилища или файл повреждён.",
    },
    adHocSigning:
      "Почти всегда это значит, что Soma обновилась. Soma подписывается ad-hoc, без платного сертификата Apple, поэтому macOS считает каждую новую сборку другим приложением — и не отдаёт другому приложению запись связки ключей, созданную прежним. Ключ и данные целы. Вставьте ключ ниже, чтобы сразу вернуться в приложение, и сохраните его копию: при следующем обновлении повторится то же самое.",
    retry: "Попробовать снова",
    quarantine: "Отложить этот файл и открыть хранилище",
    keyLabel: "Ключ восстановления",
    keyPlaceholder: "64 шестнадцатеричных символа",
    keyHint:
      "Если вы сохраняли ключ восстановления, вставьте его сюда. Ключ можно также прочитать из связки ключей самому — выполните в Терминале и подтвердите запрос:",
    unlockWithKey: "Открыть этим ключом",
    openFolder: "Показать папку с данными",
    recoverTool:
      "В любом случае файлы можно восстановить и вне приложения — утилитой soma-recover из репозитория Soma. Она читает тот же формат и никогда не пишет в эту папку.",
  },

  // Экран разблокировки (режим парольной фразы, показывается до загрузки приложения)
  unlock: {
    title: "Разблокировать Soma",
    description: "Введите парольную фразу базы данных, чтобы расшифровать данные о здоровье.",
    passphrase: "Парольная фраза",
    unlock: "Разблокировать",
    unlocking: "Разблокируем…",
    wrong: "Неверная парольная фраза — попробуйте ещё раз.",
    error: "Не удалось разблокировать базу данных.",
    forgot:
      "Забыли её? Зашифрованные данные нельзя восстановить без парольной фразы. Вы можете восстановиться из незашифрованной резервной копии или начать заново.",
  },

  // Список биомаркеров: контролы + бейдж статуса (добавлено одним блоком)
  biomarkerList: {
    allCategories: "Все категории",
    count: "{{shown}} из {{total}}",
    reset: "Сбросить фильтры",
    emptyFiltered: "Ничего не подходит под текущие фильтры: {{filters}}.",
    refRange: "Норма",
    noRefRange: "Без референсного диапазона",
    status: {
      all: "Любой статус",
      withData: "С данными",
      outOfRange: "Вне нормы",
      notOptimal: "В норме, не оптимально",
      optimal: "Оптимально",
      notEvaluated: "Не проверено",
      noData: "Без данных",
    },
    sort: {
      attention: "Сначала требующие внимания",
      name: "По названию",
      recent: "Сначала недавно измеренные",
      stale: "Сначала давно не измерялись",
      change: "Сначала сильнее всего изменились",
    },
    activeFilter: {
      query: "поиск «{{query}}»",
      status: "статус: {{value}}",
      category: "категория: {{value}}",
    },
  },
  flagBadge: {
    inRange: "в норме",
    notEvaluated: "не проверено",
    notEvaluatedHint: "Единица не распознана \u2014 значение не сверено с референсным диапазоном",
    low: "ниже нормы",
    high: "выше нормы",
    critical: "критично",
  },
  recordCard: {
    noDose: "Доза не указана",
    onset: "Начало {{date}}",
  },
} as const;
