export const ru = {
  ...
  settings: {
    title: "Настройки",
    description: "Провайдеры ИИ, профиль и экспорт данных. Всё остаётся на этом устройстве, если вы не решите иначе.",
    ...
    profile: {
      title: "Профиль",
      description: "Используется для референсных диапазонов биомаркеров с учётом пола и возраста. Пока один профиль; совместное использование нескольких профилей появится позже.",
    ...
    ai: {
      title: "ИИ-анализ",
      description: "По умолчанию отключён. Используйте свой API-ключ — он хранится в системной связке ключей, никогда в базе данных или конфигурационных файлах. Предлагаются только мультимодальные модели (зрение + PDF), поскольку пайплайн импорта читает фотографии и PDF.",
      provider: "Провайдер",
      aiDisabled: "ИИ отключён",
      model: "Модель (только мультимодальные)",
      customModel: "Пользовательский ID модели (опционально, переопределяет список — должна поддерживать зрение + PDF)",
      ...
      disclaimer: "Вывод ИИ никогда не является медицинской консультацией. Документы, которые вы импортируете, отправляются выбранному провайдеру только когда вы явно запускаете действие ИИ.",
      aiDisclaimer: "Контент создан ИИ. Не является медицинской консультацией — всегда обращайтесь к квалифицированному врачу.",
    },
  ...
  onboarding: {
    ...
    welcomeDescription: "Ваша медицинская карта на ваших условиях — анализы, лекарства и визиты в одной приватной временной шкале. Минута настройки позволит Soma читать ваши результаты относительно референсных диапазонов, которые действительно применимы к вам.",
    privacyNote: "Всё, что вы вводите, остаётся на этом устройстве — никаких аккаунтов, облака или отслеживания.",
    getStarted: "Начать",

    aboutYouTitle: "О вас",
    aboutYouDescription: "Пол и возраст определяют, какие референсные диапазоны применимы к вашим биомаркерам — поэтому эти несколько полей обязательны.",

    fineTuningTitle: "Тонкая настройка",
    fineTuningDescription: "Необязательно, но стоит того: это уточняет некоторые референсные диапазоны и помогает отслеживать цели. Пропускайте что угодно — всё можно изменить позже в настройках.",

    allSetTitle: "Всё готово",
    allSetTitleWithName: "Всё готово, {{name}}",
    allSetDescription: "Ваш профиль сохранён локально. Импортируйте отчёт о лабораторных исследованиях, записывайте что принимаете, и Soma начнёт соединять точки.",
  ...
  labs: {
    ...
    emptyDescription: "Введите результаты вручную или импортируйте PDF/фото лабораторного отчёта с помощью ИИ.",
  ...
  timeline: {
    ...
    emptyDescription: "События появятся здесь по мере добавления анализов, лекарств, визитов и диагнозов.",
  ...
  biomarkers: {
    title: "Биомаркеры",
    description: "Справочный словарь с нормальными и оптимальными диапазонами. Нажмите на биомаркер, чтобы увидеть тренд.",
  ...
  medications: {
    ...
    description: "Что вы принимаете, в какой дозе и с какого времени — можно накладывать на любой тренд биомаркеров.",
    emptyTitle: "Пока ничего не отслеживается",
    emptyDescription: "Добавьте лекарства и добавки с дозами и периодами, чтобы сопоставить их с вашими анализами.",
  ...
  backup: {
    ...
    description: "Зашифрованные снимки вашей базы данных, сохранённые в папку, которую ваш облачный клиент (iCloud Drive, Google Drive, Dropbox, OneDrive) уже синхронизирует. Soma никогда не загружает ничего сама и сохраняет только зашифрованные .somabk файлы там — ваши живые данные никогда не покидают это устройство.",
    ...
    disableNote: "Отключение только останавливает будущие резервные копии — существующие снимки и ваша парольная фраза сохраняются, поэтому вы можете повторно включить в любое время.",
    ...
    customWarning: "Эта папка не является одной из обнаруженных облачных папок. Резервные копии, сохранённые здесь, попадут в облако только если что-то на этом компьютере синхронизирует эту папку.",
    folderNotExist: "Эта папка не существует.",
    folderNotWritable: "Soma не может записывать в эту папку — выберите другую.",
    subfolderNote: "Внутри создаётся подпапка Soma Backups с README, объясняющим файлы. Туда попадают только зашифрованные снимки — никогда ваши живые данные.",

    // Step 2 - passphrase
    passphraseTitle: "Эта парольная фраза шифрует каждую резервную копию. Она хранится только в связке ключей этого устройства.",
    passphraseWarning: "Если вы потеряете это устройство и парольную фразу, ваши резервные копии не смогут быть восстановлены — никем. Сохраните её в своём менеджере паролей прямо сейчас.",
    ...
    catchupNote: "Soma также проверяет при каждом запуске и наверстывает, если резервная копия была пропущена. Сохраняются 12 новейших снимков; более старые удаляются автоматически.",
    enableAndBackup: "Включить и создать резервную копию",

    // Restore dialog
    restoreTitle: "Восстановить из резервной копии",
    restorePickDescription: "Выберите .somabk файл и введите парольную фразу, которой он был зашифрован.",
    ...
    schema: "схема",
    appHas: "в приложении",
    newerVersionError: "Эта резервная копия была создана более новой версией Soma. Сначала обновите приложение, затем восстановите.",
    olderSchemaNote: "Резервная копия использует более старую схему — она будет автоматически мигрирована после перезапуска.",
    replaceWarningTitle: "Это заменит вашу текущую базу данных и перезапустит приложение.",
    replaceWarningDescription: "Резервная копия текущей базы данных с временной меткой сохраняется рядом с ней, поэтому это можно отменить вручную при необходимости.",
  ...
  importWizard: {
    title: "ИИ-импорт",
    description: "PDF или фото → извлечённые значения → вы подтверждаете соответствие → сохранено. Ничего не записывается без вашего просмотра.",
    ...
    aiDisabledDescription: "Чтобы импортировать отчёты лабораторий из PDF или фотографий, включите ИИ-анализ и вставьте свой API-ключ в настройках. Ваши документы отправляются только выбранному провайдеру и только когда вы запускаете импорт.",
  ...
  dashboard: {
    ...
    recentActivity: {
      description: "Добавьте свою первую панель анализов, лекарство или визит к врачу, чтобы начать.",
    },
  },
  ...
  visitDetail: {
    ...
    deleteVisitDescription: "Назначения этого визита удаляются; связанные диагнозы сохраняются, но отвязываются.",
