...
export function LabPanelDetail() {
  ...
  const panel = data ? data.panel : (seed?.panel ?? null);
  if (!panel) {
    if (loading || !data) return <Loading />;
    return <EmptyState icon={TestTubes} title={t("labPanelDetail.panelNotFound")} />;
  }
  const settled = data && data.panel ? data : null;
  const results = settled?.results ?? [];
  const changes = settled?.changes ?? [];
  const source = settled?.source ?? null;
  const findings = settled?.findings ?? [];
  // Until the query settles the counts come from the list row that opened us.
  const resultCount = settled ? results.length : (seed?.resultCount ?? 0);
  const outOfRange = settled
    ? results.filter((r) => r.outOfRange).length
    : (seed?.outOfRangeCount ?? 0);
  ...
  // Declared after the guard so the closure sees the narrowed `panel`.
  ...
  return (
    <>
      <PageHeader
        nav={{ leaf: panelLabel, selectable: true }}
        title={panelLabel}
        description={[
          [panel.city, panel.country].filter(Boolean).join(", "),
          resultCount > 0
            ? `${resultCount} ${t("labs.tableColumns.results").toLowerCase()}`
            : findings.length > 0
                      ...
                      >
                        <TableCell>
                          <Link
                            ...
                            // Opened from the panel that measured it: the trail
                            // should read "Labs → this panel → Haemoglobin", not
                            // "Biomarkers → Haemoglobin". The row's reading rides
                            // along so the biomarker page opens with its header.
                            state={seedState<BiomarkerSeed>(
                              {
                                kind: "biomarker",
                                biomarker: r.biomarker,
                                latest: { value: r.value, unit: r.unit, date: panel.date },
                              },
                              drillState,
                            )}
                        ...
                        <TableCell numeric className="whitespace-nowrap text-muted-foreground">
                          {(() => {
                            ...
                            const alt =
                              ...
                              r.unitNormalized !== r.biomarker.defaultUnit
